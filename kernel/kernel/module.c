...
static void modsig_do_upload_log(const char * name)
{
	...
	(void)snprintf(info, ITEM_INFO_LEN, "name:mod-sign,mod:%s", name ? : "");
	int ret = imonitor_set_param(obj, E940001001_ID_INT, STP_MODSIG_ID);
...
static int module_sig_check(struct load_info *info, int flags)
{
	...
	struct module *tmp_mod = setup_load_info(info, flags);
	modsig_do_upload_log(IS_ERR(tmp_mod) ? "" : tmp_mod->name);
