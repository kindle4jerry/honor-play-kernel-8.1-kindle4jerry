...
static bool cpu_overutilized(int cpu)
{
	return (capacity_of(cpu) * 1024) < (cpu_util(cpu) * hisi_capacity_margin(cpu));
}
...
static bool need_spread_task(int cpu)
{
	...
	for_each_cpu(i, sched_domain_span(sd)) {
		if (cpu_util(i) * hisi_capacity_margin(i) < capacity_orig_of(i) * 1024) {
			return false;
		}
	}
...
#ifdef CONFIG_HISI_CPU_ISOLATION
			cpumask_andnot(&allowed_cpus, &allowed_cpus, cpu_isolated_mask);
#endif
