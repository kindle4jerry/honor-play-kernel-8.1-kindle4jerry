...
static int f2fs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	if (logusertype != OVERSEA_USER && logusertype != OVERSEA_COMMERCIAL_USER) {
		...
		if (should_monitor_file(dentry) || is_multimedia_file(dentry->d_name.name, "mp4")) {
			...
			if (!is_multimedia_file(dentry->d_name.name, "mp4")) {
				...
				pr_err("F2FS-Video-Monitor:pkg = %s PID = %d uid = %d\n",
					pkg, task_pid_nr(tsk), uid);
			}
			kfree(pkg);
		}
	}
