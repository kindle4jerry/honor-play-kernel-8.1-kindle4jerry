...
static int wp_page_copy(struct mm_struct *mm, struct vm_area_struct *vma,
			...
			pte_t orig_pte, struct page *old_page, bool is_swap)
{
	...
	if (is_zero_pfn(pte_pfn(orig_pte))) {
		...
		gfp_t gfp_flag = GFP_HIGHUSER_MOVABLE;
		if (!is_swap)
			gfp_flag |= ___GFP_CMA;
		new_page = alloc_page_vma(gfp_flag, vma, address);
		if (!new_page)
			goto oom;
...
static int do_wp_page(struct mm_struct *mm, struct vm_area_struct *vma,
		unsigned long address, pte_t *page_table, pmd_t *pmd,
		spinlock_t *ptl, pte_t orig_pte, bool is_swap)
	__releases(ptl)
{
	struct page *old_page;
		...
		unsigned int flags, pte_t orig_pte)
{
	...
	if (flags & FAULT_FLAG_WRITE) {
		ret |= do_wp_page(mm, vma, address, page_table, pmd, ptl, pte, true);
		if (ret & VM_FAULT_ERROR)
			ret &= VM_FAULT_ERROR;
		goto out;
	}
