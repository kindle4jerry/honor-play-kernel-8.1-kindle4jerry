...
do{\
	if(get_rtc_dsm_client()){\
		if(!dsm_client_ocuppy(get_rtc_dsm_client())){\
			...
			printk(KERN_INFO "[RTC DSM]rtc dsm report err_no = %d\n",err_no);\
		}else\
		    printk(KERN_ERR "[RTC DSM]rtc dsm is busy err_no = %d\n",err_no);\
	}else\
...
unsigned long hisi_pmu_rtc_readcount(void)
{
	...
	printk(KERN_INFO "[%s] time = %u, lastTime = %u\n", __func__, time, lastTime);
	if (lastTime > time){
		/* DMD RTC PRINT INFO */
