...
static int atfisp_loadfw(void)
{
    ...
    pr_info("[%s] + %s.(0x%lx, 0x%x), init.%x, sec_verify.%x\n", __func__, loadinfo.partion_name, loadinfo.image_addr, loadinfo.image_size, dev->ispops->refs_fw, dev->sec_verify);
    if (!dev->sec_verify)
    {
    ...
    else
    {
        if ((ret = bsp_load_sec_img(&loadinfo)) < 0) {
            pr_err("[%s] Failed : bsp_load_sec_image.%d, %s.(0x%lx, 0x%x)\n", __func__, ret, loadinfo.partion_name, loadinfo.image_addr, loadinfo.image_size);
            return ret;
        }
    }
    ...
    pr_info("[%s] - bsp_load_image.%d, %s.(0x%lx, 0x%x), init.%x, sec_verify.%x\n", __func__, ret, loadinfo.partion_name, loadinfo.image_addr, loadinfo.image_size, dev->ispops->refs_fw, dev->sec_verify);
	return 0;
}
...
static int smmu_err_addr_init(void)
{
    ...
    pr_debug("[%s] sec_smmuerr_addr.0x%llx\n", __func__, dev->sec_smmuerr_ion->paddr);
    return 0;
}
...
static int set_share_pararms(void)
{
    ...
    if ((ret = atfisp_setparams(dev->atfshrd_paddr)) < 0) {
        pr_err("[%s] atfisp_setparams.%d, addr.0x%llx\n", __func__, ret, dev->atfshrd_paddr);
        kfree(dev->atfshrd_vaddr);
        return -ENODEV;
    }
    pr_debug("[%s] atfisp_setparams.%d, (type.0x%x, atfshrd_paddr.0x%llx)\n", __func__, ret, HISP_PARAMS_SHRDLIST, dev->atfshrd_paddr) ;
    return 0;
}
/*lint -save -e429*/
static int free_secmem_ion(void *data)
{
    ...
    if((hisi_secmem_ion->ion_client      == NULL) || \
       ...
       {
        pr_err("[%s] ion_client.%pK, boot_ion_handle.%pK, fw_ion_handle.%pK,\n", __func__, hisi_secmem_ion->ion_client, \
                hisi_secmem_ion->boot_ion_handle, hisi_secmem_ion->fw_ion_handle);
        mutex_unlock(&dev->sec_mem_mutex);
        return -ENODEV;
        }
