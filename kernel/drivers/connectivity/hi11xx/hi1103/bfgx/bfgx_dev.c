...
int32 bfgx_open_cmd_send(uint32 subsys)
{
    ...
    if (!timeleft)
    {
        ...
        if (BFGX_GNSS == subsys)
        {
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_GNSS_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_OPEN_THREAD));
        }
        return -ETIMEDOUT;
    }
...
int32 bfgx_close_cmd_send(uint32 subsys)
{
    ...
    if (!timeleft)
    {
        ...
        if (BFGX_GNSS == subsys)
        {
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_GNSS_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_CLOSE_THREAD));
        }
        return -ETIMEDOUT;
    }
...
STATIC int32 hw_ir_open(struct inode *inode, struct file *filp)
{
    ...
    /* judge ir only mode*/
    if ((true == wlan_is_shutdown()) && (true == bfgx_is_shutdown())
...
{
    static int32 max_cnt[BFGX_THREAD_BOTTOM] ={0,0,0,0,0};

    PS_PRINT_INFO("[subs id:%d]arm timeout trigger cnt[%d]", subs, max_cnt[subs]);
    if ((subs >= BFGX_THREAD_BOTTOM) || (subs < THREAD_BT)){
        return -EINVAL;
    }
    if (max_cnt[subs] <= ARP_TIMEOUT_MAX_TIMES) {
        plat_exception_handler(SUBSYS_BFGX, THREAD_GNSS, ARP_TIMEOUT);
        max_cnt[subs]++;
    }else {
