        ...
        if(subsys_type == SUBSYS_BFGX) {
            switch (exception_type)
            {
                 ...
                 case LAST_WORD:
                     ...
                     break;
                 case BEAT_HEART_TIMEOUT:
                     RDR_EXCEPTION(MODID_CONN_BFGX_BEAT_TIMEOUT);
                     break;
     //            case TIMER_TIMEOUT:
        //         case BFGX_POWER_ON_FAIL:
...
int32 hisi_conn_save_stat_info(char* buf,int32 index ,int32 limit)
{
    ...
    for( i = 0; i < MODID_CONN_ARRAY_LEN; i++)  {
            index += snprintf( buf + index, limit - index,"   id:%-10d  upload_en:%d    upload_cnt:%llu    happen_cnt:%llu    interva_stime:%llus   lastuploadtime:%llus\n",
                                                                                                i,
                                                                                               ...
                                                                                               hisi_conn_modid_cfg[i].stat_info.lastuploadtime);
    }
    return index;
}
...
int hisi_conn_rdr_init(void)
{
    ...
    PS_PRINT_INFO("hisi_conn_rdr_init all succ\n");
    initdone = 1;
    return 0;
