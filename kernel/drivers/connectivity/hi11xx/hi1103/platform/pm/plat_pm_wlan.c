...
oal_int32 wlan_pm_open(oal_void)
{
    ...
    if(NULL == pst_bus)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_WIFI_DRV_ERROR_POWER_ON_NON_BUS));
        return OAL_FAIL;
    }
    ...
    if(OAL_PTR_NULL == g_pst_custom_process_func.p_custom_cali_func)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_WIFI_DRV_ERROR_POWER_ON_NO_CUSTOM_CALL));
        mutex_unlock(&pm_data->host_mutex);
        return OAL_FAIL;
    }
    ...
    if (OAL_SUCC == g_pst_custom_process_func.p_custom_cali_func())
    {
        ...
        if ((g_uc_autocali_switch == OAL_FALSE) || (g_uc_custom_cali_done == OAL_FALSE))
        {
            ...
            if(0 == oal_wait_for_completion_timeout(&pst_bus->st_device_ready, (oal_uint32)OAL_MSECS_TO_JIFFIES(HOST_WAIT_BOTTOM_INIT_TIMEOUT)))
            {
                ...
                CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_WIFI_DRV_ERROR_POWER_ON_CALL_TIMEOUT));
...
oal_uint32 set_bfgx_mem_check_mdelay(int32 mdelay)
{
    ...
    OAL_IO_PRINT("bfgx_mem_check_mdelay::set delay:%dms!!\n",bfgx_mem_check_mdelay);
    return 0;
}
...
oal_uint wlan_pm_init_dev(void)
{
    ...
    if (-ETIME == ret)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_WIFI_DRV_ERROR_POWER_ON_SDIO_NO_READY));
        return OAL_FAIL;
    }
...
oal_uint wlan_pm_wakeup_dev(oal_void)
{
    ...
    if(OAL_WARN_ON(NULL == pst_bus))
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_WAKEUP_DEV));
        return OAL_FAIL;
    }
