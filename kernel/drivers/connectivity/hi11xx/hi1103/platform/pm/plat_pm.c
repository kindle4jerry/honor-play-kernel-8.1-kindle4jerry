...
#define RETRY_TIMES (3)
    ...
    for (i = 0; i < RETRY_TIMES; i++)
    {
        ...
        do
        {
            ...
            timeleft = wait_for_completion_timeout(&pm_data->dev_ack_comp, msecs_to_jiffies(1000));
...
#define MAX_SENSORHUB_LOOPCNT (30)
    ...
    if (!g_b_ir_only_mode)
    {
        ...
        if((!bfgx_other_subsys_all_shutdown(BFGX_GNSS))||(GNSS_AGREE_SLEEP == atomic_read(&pm_data->gnss_sleep_flag)))
        {
...
int32 host_wkup_dev(void)
{
    ...
    if (!timeleft)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_GNSS_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_WAKEUP_DEV));
...
int32 bfgx_dev_power_on(void)
{
    ...
    if (!timeleft)
    {
        ...
        if (wlan_is_shutdown())
        {
            ...
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_BCPU_BOOTUP));
...
int firmware_download_function(uint32 which_cfg)
{
    ...
    if (OAL_SUCC != ret)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_FAIL_FIRMWARE_DOWN));
        return -FAILURE;
    }
    ...
    if (!test_bit(FIRMWARE_CFG_INIT_OK, &pm_data->firmware_cfg_init_flag))
    {
        ...
        if (ret)
        {
            ...
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_CFG_FAIL_FIRMWARE_DOWN));
            return ret;
        }
    ...
    if (ret < 0)
    {
        ...
        {
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_GNSS_DRV(CHR_GNSS_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_FIRMWARE_DOWN));
         }
        else
        {
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_FIRMWARE_DOWN));
        }
        return -FAILURE;
    }
...
int32 wlan_power_on(void)
{
    ...
    if(OAL_TRUE == hcc_bus_exception_is_busy(hcc_get_current_110x_bus()))
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_OPEN, CHR_WIFI_DRV_ERROR_POWER_ON_NON_BUS));
        return -FAILURE;
    }
...
wifi_power_fail:
    ...
    CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_OPEN, CHR_WIFI_DRV_ERROR_POWER_ON));
...
int32 device_mem_check(unsigned long long *time)
{
    ...
    ret = firmware_get_cfg(g_auc_cfg_path[RAM_REG_TEST_CFG], RAM_REG_TEST_CFG);
    if(ret)
    {
    ...
    if(SUCCESS == ret)
    {
        ...
        else
        {
            ...
            firmware_get_cfg(g_auc_cfg_path[RAM_REG_TEST_CFG], RAM_REG_TEST_CFG);
            goto exit_error;
        }
    }
    firmware_get_cfg(g_auc_cfg_path[RAM_REG_TEST_CFG], RAM_REG_TEST_CFG);
