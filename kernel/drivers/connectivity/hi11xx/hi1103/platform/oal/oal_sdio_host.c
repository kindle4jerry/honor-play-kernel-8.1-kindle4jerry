...
oal_int32 oal_sdio_reinit(hcc_bus* pst_bus)
{
    ...
    if(ret < 0)
    {
        ...
        ssi_dump_device_regs(SSI_MODULE_MASK_COMM|SSI_MODULE_MASK_SDIO);
    ...
    if (ret)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT));
...
oal_int32 oal_sdio_data_sg_irq(struct oal_sdio *hi_sdio)
{
    ...
    OAL_BUG_ON(NULL == hi_sdio->pst_bus->bus_ops.rx);
...
oal_void oal_sdio_isr(struct sdio_func *func)
{
    ...
    do
    {
        ...
        if(OAL_UNLIKELY(ret))
        {
            ...
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT_ISR));
        }
        sdio_release_host(hi_sdio->func);
        sdio_claim_host(hi_sdio->func);
...
oal_int32 oal_sdio_rxdata_proc(struct oal_sdio *hi_sdio)
{
    ...
    if(OAL_UNLIKELY(ret))
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT_RX_DATA_PROC));
    }
    sdio_release_host(hi_sdio->func);
...
oal_void oal_sdio_interrupt_unregister(struct oal_sdio *hi_sdio)
{
    if (hisdio_intr_mode)
    {
        ...
        //TODO
        //oal_unregister_gpio_intr(hi_sdio->pst_bus);
    }
    else
    {
        sdio_claim_host(hi_sdio->func);
...
failed_sdio_alloc:
    ...
    CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT_PROB_FAIL));
                        ...
                        oal_uint32 rw_sz)
{
    ...
    if(OAL_UNLIKELY(ret))
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_WR_FAIL));
    }
    sdio_release_host(func);
    return ret;
}
...
OAL_STATIC oal_int32 oal_sdio_gpio_irq(hcc_bus *hi_bus, oal_int32 irq)
{
    ...
    else
    {
        ...
        OAL_BUG_ON(!hi_bus->pst_pm_callback->pm_wakeup_host);
...
oal_int32 oal_wifi_platform_load_sdio(oal_void)
{
    ...
    if(OAL_SUCC != oal_sdio_trigger_probe())
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DEV_EVENT_CHIP, CHR_WIFI_DEV_ERROR_SDIO_ENUM));
