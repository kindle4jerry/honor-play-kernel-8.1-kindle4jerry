...
oal_int32 hcc_queues_pkts_info_print(struct hcc_handler* hcc,char* buf, oal_int32 buf_len)
{
    ...
    //for(j = 0; j < HCC_QUEUE_COUNT;j++)
    {
        ...
        ret +=  snprintf(buf + ret , buf_len - ret,"rx queue:%4d,low wl:%u,high wl:%u, enabled:%d\n",
                        DATA_LO_QUEUE,
                        pst_hcc_queue->flow_ctrl.low_waterline,
...
OAL_STATIC oal_uint32 hcc_test_get_buf_remain_len(oal_uint32 buf_len, oal_uint32 req_len)
{
    if(buf_len >= req_len)
    {
        buf_len -= req_len;
...
ssize_t hcc_test_print_thoughput(char*buf, oal_uint32 buf_len)
{
    ...
    ret +=  snprintf(buf + ret , hcc_test_get_buf_remain_len(buf_len, ret), "Test_Mode: %s\n",mode_str);
    ret +=  snprintf(buf + ret , hcc_test_get_buf_remain_len(buf_len, ret), "Actual sent %d pkts, request %llu bytes\n",
                                              g_hcc_test_event->test_data.pkt_sent,
                                              ((oal_uint64)g_hcc_test_event->test_data.pkt_sent)*(oal_uint64)g_hcc_test_event->test_data.pkt_len);
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "Actual rcvd %d pkts, request %llu bytes\n", g_hcc_test_event->test_data.pkt_rcvd,
                                              ((oal_uint64)g_hcc_test_event->test_data.pkt_rcvd)*(oal_uint64)g_hcc_test_event->test_data.pkt_len);
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "PayloadSend %llu bytes, ActualSend  %llu bytes\n",
                                                            g_hcc_test_event->test_data.total_sent_bytes,
                                                            g_hcc_test_event->test_data.trans_info.total_h2d_trans_bytes);
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "PayloadRcvd %llu bytes, ActualRecv  %llu bytes\n",
                                                            g_hcc_test_event->test_data.total_rcvd_bytes,
                                                            g_hcc_test_event->test_data.trans_info.total_d2h_trans_bytes);
    /*SDIO通道利用率*/
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "Hcc Utilization Ratio %llu ‰\n",
                            hcc_test_utilization_ratio_gen(g_hcc_test_event->test_data.total_sent_bytes + g_hcc_test_event->test_data.total_rcvd_bytes,
                                                            g_hcc_test_event->test_data.trans_info.total_h2d_trans_bytes +
                                                             g_hcc_test_event->test_data.trans_info.total_d2h_trans_bytes));
    /*发送方向的丢包率*/
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "TxPackageLoss %u ‰, pkt_sent: %d actual_tx_pkts: %u\n",
    ...
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "Requet Generate %d pkts\n", g_hcc_test_event->test_data.pkt_gen);

    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "Per-package Length %d\n", g_hcc_test_event->test_data.pkt_len);
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "TranserTimeCost %llu Seconds, %llu microsecond\n",
                                        g_hcc_test_event->test_data.trans_time,g_hcc_test_event->test_data.trans_time_us);
    ret +=  snprintf(buf + ret,  hcc_test_get_buf_remain_len(buf_len, ret), "Throughput %u Mbps\n",(oal_int32)g_hcc_test_event->test_data.throughput);
...
oal_int32 conn_test_wifi_chan_loop(char *param)
{
    ...
    if(OAL_TRUE == oal_pcie_110x_working_check())
    {
        ...
        if(INI_SUCC == get_cust_conf_int32(INI_MODU_PLAT, "pcie_min_throught", &pcie_min_throught))
        {
...
#ifdef CONFIG_KIRIN_PCIE_NOC_NOTIFY
