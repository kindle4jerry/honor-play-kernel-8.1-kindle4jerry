...
irqreturn_t oal_pcie_intx_isr(int irq, void*dev_id)
{
    ...
    if(OAL_UNLIKELY(oal_pcie_transfer_done(pst_pci_lres->pst_pci_res) < 0))
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_INTX_ISR_PCIE_LINK_DOWN));
        /*关闭低功耗*/
...
OAL_STATIC oal_void oal_pcie_linkdown_callback(struct kirin_pcie_notify *noti)
{
    ...
    if(0 == board_get_wlan_wkup_gpio_val())
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_WKUP_GPIO_PCIE_LINK_DOWN));
    }
    else
    {
...
OAL_STATIC oal_int32 oal_pcie_device_wake_powerup_handler(void* data)
{
    ...
    OAL_REFERENCE(data);
    PCI_PRINT_LOG(PCI_LOG_INFO, "pcie_resume_powerup pull up gpio");
...
OAL_STATIC oal_int32 oal_pcie_resume(oal_pci_dev_stru *pst_pci_dev)
{
    ...
    if(ret)
    {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_RESUME_FIRMWARE_DOWN));
        return OAL_SUCC;
    }
...
oal_int32 oal_disable_pcie_irq(oal_pcie_linux_res* pst_pci_lres)
{
    ...
    if(0 == pst_pci_lres->irq_stat)
    {
        if(!pst_pci_lres->st_msi.is_msi_support)
        {
            ...
            {
                disable_irq(pst_pci_lres->pst_pcie_dev->irq);
            }
        }
        else
        {
...
oal_void oal_pcie_voltage_bias_param_init(oal_void)
{
    ...
    if(INI_FAILED == get_cust_conf_string(INI_MODU_PLAT, "pcie_vol_bias_param", bias_param, sizeof(bias_param) - 1))
    {
        return;
    }
