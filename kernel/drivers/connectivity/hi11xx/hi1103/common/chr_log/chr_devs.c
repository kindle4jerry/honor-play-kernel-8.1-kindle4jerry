...
int32 chr_wifi_dev_tx_handler(uint32 errno)
{
    ...
    OAL_IO_PRINT("chr_wifi_dev_tx_handler::hcc tx is succ,errno %u\n",errno);
...
int32 chr_wifi_tx_handler(uint32 errno)
{
    ...
    {
        CHR_ERR("chr_wifi_tx_handler::wifi tx failed,errno[0x%x],ret1[%u],ret2[%u]",errno,ret1,ret2);
        return -CHR_EFAIL;
    }
...
int32 chr_bfg_dev_tx_handler(uint32 ul_errno)
{
    ...
    *(uint32*)(skb->data+ sizeof(struct ps_packet_head)) = ul_errno;
    ...
    CHR_WARNING("chr_bfg_dev_tx_handler::hcc tx is succ,errno %u\n",ul_errno);
