...
oal_uint32 check_headroom_add_length(mac_tx_ctl_stru *pst_tx_ctl, frw_event_type_enum_uint8 en_nest_type, oal_uint8 uc_nest_sub_type, oal_uint8 uc_cb_length)
{
                                    ...
                                    oal_uint32 queue_id)
{
    ...
    //OAM_ERROR_LOG3(0, OAM_SF_ANY, "{hmac_hcc_tx_netbuf::ul_hcc_head_len[%d],ul_headroom_add[%d],expand data[%x]}",ul_hcc_head_len,ul_headroom_add,OAL_NETBUF_DATA(pst_netbuf));
    ...
    //OAM_ERROR_LOG3(0, OAM_SF_ANY, "{hmac_hcc_tx_netbuf::expand data[%x],old[%x]new[%x]}",OAL_NETBUF_DATA(pst_netbuf),ul_netbuf_old_addr,ul_netbuf_new_addr);

    /* 未对齐时在host侧做数据搬移，此处牺牲host，解放device */
    if (ul_addr_offset)
    {
        ul_head_room = (oal_int32)oal_netbuf_headroom(pst_netbuf);
        //OAM_WARNING_LOG2(0, OAM_SF_ANY, "{hmac_hcc_tx_netbuf 4 bytes offset[%d],headroom[%d]}",ul_addr_offset,ul_head_room);
...
{
    return hmac_hcc_tx_event_payload_to_netbuf(pst_event_mem, (oal_uint32)((frw_get_event_stru(pst_event_mem))->st_event_hdr.us_length));
}
