    ...
    switch(pst_param->en_vap_mode)
    {
        case WLAN_VAP_MODE_BSS_AP:
         ...
         /* 组播转单播初始化函数 */
            ...
            FRW_TIMER_CREATE_TIMER(&(pst_hmac_vap->st_edca_opt_timer), hmac_edca_opt_timeout_fn, pst_hmac_vap->ul_edca_opt_time_ms, pst_hmac_vap, OAL_TRUE, OAM_MODULE_ID_HMAC, pst_hmac_vap->st_vap_base_info.ul_core_id);
        ...
        #ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
            mac_mib_set_AmsduPlusAmpduActive(&pst_hmac_vap->st_vap_base_info,(oal_uint8)!!hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_AP_TX_AMSDU));
            pst_hmac_vap->bit_rx_ampduplusamsdu_active = (oal_uint8)!!hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_AP_RX_AMSDU);
        #endif
#endif
            break;
...
#endif
    {
        pst_hmac_vap->bit_bcn_table_switch = OAL_TRUE;
    }
                ...
                hmac_user_stru                   *pst_hmac_user_sta)
{
    ...
    if (OAL_PTR_NULL != puc_ie)
    {
        ...
        {
            OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_vap_check_ht_capabilities_ap::STA support ht capability but support none space_stream.}");
            /* 对端ht能力置为不支持 */
            mac_user_set_ht_capable(&(pst_hmac_user_sta->st_user_base_info), OAL_FALSE);
        }
    }
                ...
                oal_bool_enum                en_prev_asoc_ht)
{
    ...
    ***************************************************************************/
    ...
    OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_search_ht_cap_ie_ap::en_40M_intol_user[%d].}", pst_mac_vap->en_40M_intol_user);
...
oal_void hmac_set_ampdu_hw_worker(oal_delayed_work *pst_work)
{
    ...
    /***************************************************************************
            抛事件到DMAC层, 同步DMAC数据
     ***************************************************************************/
    ul_ret = hmac_config_send_event(&pst_hmac_vap->st_vap_base_info, WLAN_CFGID_AMPDU_TX_ON, OAL_SIZEOF(mac_cfg_ampdu_tx_on_param_stru), (oal_uint8*)&st_ampdu_tx_on);
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,"hmac_set_ampdu_hw_worker::hmac_config_send_event fail(%u)",ul_ret);
    }
...
oal_void hmac_del_virtual_inf_worker(oal_work_stru *pst_del_virtual_inf_work)
{
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_P2P,"{hmac_del_virtual_inf_worker:: net_dev is null}");
        return;
    }
...
oal_uint32 hmac_restart_all_work_vap(oal_uint8 uc_chip_id)
{
    ...
    for (uc_dev_idx = 0; uc_dev_idx < pst_mac_chip->uc_device_nums; uc_dev_idx++)
    {
        ...
        for (uc_vap_idx = 0; uc_vap_idx < pst_mac_dev->uc_vap_num; uc_vap_idx++)
        {
            ...
            if (ul_ret != OAL_SUCC)
            {
                OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG, "{hmac_restart_all_work_vap::hmac_config_down_vap failed[%d].}", ul_ret);
                return ul_ret;
            }
            /* down netdev */
