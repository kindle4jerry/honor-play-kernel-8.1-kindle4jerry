...
oal_void  hmac_update_join_req_params_2040(mac_vap_stru *pst_mac_vap, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{hmac_update_join_req_params_2040::en_channel_bandwidth=%d, mac vap bw[%d].}",
...
oal_uint32  hmac_sta_wait_auth(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    else
    {
        ...
        pst_hmac_user_ap = mac_res_get_hmac_user(pst_hmac_sta->st_vap_base_info.us_assoc_vap_id);
...
oal_uint32  hmac_sta_wait_auth_seq2_rx(hmac_vap_stru *pst_sta, oal_void *pst_msg)
{
        ...
        && (WLAN_WITP_AUTH_AUTOMATIC != mac_mib_get_AuthenticationMode(&pst_sta->st_vap_base_info)))
    {
        OAM_WARNING_LOG2(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                         "{hmac_sta_wait_auth_seq2_rx::rcv unexpected auth alg[%d/%d].}", us_auth_alg, mac_mib_get_AuthenticationMode(&pst_sta->st_vap_base_info));
    }
...
oal_uint32  hmac_sta_wait_asoc(hmac_vap_stru *pst_sta, oal_void *pst_msg)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG0(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_sta_wait_asoc::hmac_user_set_asoc_req_ie failed}");
        oal_netbuf_free(pst_asoc_req_frame);
        return OAL_FAIL;
    }
...
OAL_STATIC oal_void  hmac_cfg80211_prepare_listen_req_param(mac_scan_req_stru *pst_scan_params, oal_int8 *puc_param)
{
    ...
    if (OAL_SUCC != mac_get_channel_idx_from_num(pst_channel_tmp->en_band, pst_channel_tmp->uc_chan_number, &(pst_channel_tmp->uc_chan_idx)))
    {
...
oal_uint32 hmac_sta_not_up_rx_mgmt(hmac_vap_stru *pst_hmac_vap_sta, oal_void *p_param)
{
    ...
    {
        OAM_ERROR_LOG3(0, OAM_SF_RX, "{hmac_sta_not_up_rx_mgmt::puc_mac_hdr null, vap_id %d,us_frame_len %d, uc_mac_header_len %d}",
                    ...
                    hmac_user_stru          *pst_hmac_user)
{
    ...
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_WARNING_LOG1(0,OAM_SF_ASSOC,"{hmac_sta_up_update_he_edca_params::mac_res_get_dev fail.device_id[%u]!}",pst_hmac_sta->st_vap_base_info.uc_device_id);
        return;
    }
                    ...
                    hmac_user_stru          *pst_hmac_user)
{
    ...
    if (OAL_PTR_NULL != puc_ie)
    {
        ...
        hcc_host_update_vi_flowctl_param(mac_mib_get_QAPEDCATableCWmin(&pst_hmac_sta->st_vap_base_info, WLAN_WME_AC_BE),
...
oal_uint32  hmac_sta_up_update_rrm_capability(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user, oal_uint8 *puc_payload, oal_uint32 us_rx_len)
{
    ...
    puc_ie = mac_find_ie(MAC_EID_RRM, puc_payload, (oal_int32)us_rx_len);
...
oal_uint32 hmac_process_assoc_rsp(hmac_vap_stru *pst_hmac_sta,
                                  hmac_user_stru *pst_hmac_user,
                                  oal_uint8 *puc_mac_hdr,
                                  ...
                                  oal_uint16 us_msg_len)
{
    ...
    hmac_sta_up_update_edca_params(puc_payload, us_msg_len, pst_hmac_sta, mac_get_frame_sub_type(puc_mac_hdr), pst_hmac_user);
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC, "{hmac_process_assoc_rsp::rates mismatch ret[%d].}", ul_ret);
    }
    ...
    /* 打桩用于调试，只要检测到相应的hilinkie即认为用户支持4地址 */
        ...
        {
            if (OAL_PTR_NULL != mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HAUWEI_4ADDR, puc_payload, us_msg_len))
            {
                pst_hmac_user->uc_is_wds = OAL_TRUE;
                OAM_WARNING_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_ap_up_rx_asoc_req::user surpport 4 address.}");
            }
    ...
    if (pst_hmac_user->st_user_base_info.st_vht_hdl.en_vht_capable == OAL_FALSE)
    {
        ...
        puc_vendor_vht_ie = mac_find_vendor_ie(MAC_WLAN_OUI_BROADCOM_EPIGRAM, MAC_WLAN_OUI_VENDOR_VHT_TYPE, puc_payload, us_msg_len);
    ...
    uc_avail_mode = g_auc_avail_protocol_mode[pst_mac_vap->en_protocol][pst_hmac_user->st_user_base_info.en_protocol_mode];
    ...
    /* 同步用户的带宽能力 */
    mac_user_set_bandwidth_info(pst_mac_user, mac_vap_bw_mode_to_bw(en_sta_new_bandwidth), mac_vap_bw_mode_to_bw(en_sta_new_bandwidth));

    /* 获取用户与VAP空间流交集 */
    ul_ret = hmac_user_set_avail_num_space_stream(pst_mac_user, pst_mac_vap->en_vap_rx_nss);
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,"{hmac_process_assoc_rsp:ap max nss is [%d]}",pst_hmac_user->st_user_base_info.en_user_num_spatial_stream);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
...
oal_uint32 hmac_sta_sync_vap(hmac_vap_stru *pst_hmac_vap, mac_channel_stru *pst_channel, wlan_protocol_enum_uint8 en_protocol)
{
    ...
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "{hmac_sta_sync_vap::hmac_config_send_event failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_sta_wait_asoc_rx(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    if(OAL_SUCC == ul_rslt)
    {
        OAM_WARNING_LOG1(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_sta_wait_asoc_rx:: after hmac_check_ap_channel_follow_sta. ap channel change to %d}",uc_ap_follow_channel);
    }
#endif
...
{
    oal_uint32 uc_idx;
    oal_uint8 auc_min_rate_idx[2];/*第一个存储11b协议对应的速率，第二个存储11ag协议对应的速率*/
...
oal_uint32 hmac_sta_update_join_req_params(hmac_vap_stru *pst_hmac_vap, hmac_join_req_stru *pst_join_req)
{
    ...
    if (OAL_SWITCH_OFF == pst_hmac_vap->bit_sta_protocol_cfg)
    {
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "{hmac_sta_update_join_req_params::mac_vap_pro[%d], cfg_pro[%d]}", pst_mac_vap->en_protocol, st_cfg_mode.en_protocol);
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC, "{hmac_sta_update_join_req_params::ap nss[%d],ap band[%d],ap ht cap[%d],ap vht cap[%d].}",
    ...
    pst_mac_vap->st_channel.en_bandwidth   = WLAN_BAND_WIDTH_20M;
#endif
    ul_ret = mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band, pst_mac_vap->st_channel.uc_chan_number, &(pst_mac_vap->st_channel.uc_chan_idx));
    if (OAL_SUCC != ul_ret)
    {
    ...
    if (WLAN_VAP_MODE_BSS_STA == pst_hmac_vap->st_vap_base_info.en_vap_mode)
    {
        ...
        if (OAL_SUCC != oal_notice_sta_join_result(pst_hmac_vap->st_vap_base_info.uc_chip_id, OAL_FALSE))
        {
            OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                "{hmac_sta_handle_disassoc_rsp::oal_notice_sta_join_result fail.}");
        }
#endif
    }
...
OAL_STATIC oal_uint32  hmac_sta_up_rx_beacon(hmac_vap_stru *pst_hmac_vap_sta, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 更新edca参数 */
    hmac_sta_up_update_edca_params(puc_frame_body + us_frame_offset, us_frame_len - us_frame_offset, pst_hmac_vap_sta, uc_frame_sub_type, pst_hmac_user);
