...
{
    pst_bss_dscr->puc_rsn_ie = mac_find_ie(MAC_EID_RSN, puc_frame_body, (oal_int32)(us_frame_len));
    pst_bss_dscr->puc_wpa_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, puc_frame_body, (oal_int32)(us_frame_len));
}
...
OAL_STATIC oal_void  hmac_scan_update_bss_list_11n(mac_bss_dscr_stru *pst_bss_dscr, oal_uint8 *puc_frame_body, oal_uint16 us_frame_len)
{
    ...
    puc_ie = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_EID_11NTXBF, puc_frame_body, (oal_int32)us_frame_len);
    if (OAL_PTR_NULL != puc_ie)
    {
                                                                 ...
                                                                 oal_uint16           us_frame_len)
{
    ...
    if ((OAL_PTR_NULL != puc_ie) && (puc_ie[1] >= MAC_HE_CAP_MIN_LEN))
    {
        ...
        {
            if(st_he_cap_value.st_he_phy_cap.bit_channel_width_set & 0x4)/*B3-5G 80+80MHz*/
            {
                pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_80PLUS80;
            }
            else if(st_he_cap_value.st_he_phy_cap.bit_channel_width_set & 0x2)/*B2-160MHz*/
            {
                pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_160M;
            }
            else if(st_he_cap_value.st_he_phy_cap.bit_channel_width_set & 0x1)/*B1-*/
            {
                pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_40M;
            }
            else
...
oal_uint8 hmac_scan_check_chan(oal_netbuf_stru *pst_netbuf, hmac_scanned_bss_info *pst_scanned_bss)
{
    ...
    puc_ie_start_addr = mac_find_ie(MAC_EID_HT_OPERATION, puc_frame_body + us_offset, us_frame_body_len - us_offset);
...
oal_uint32 hmac_scan_rrm_proc_save_bss(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "{hmac_scan_rrm_proc_save_bss::pst_rrm_info null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
#endif
    /*************************************************************************/
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{hmac_scan_rrm_proc_save_bss::pst_mac_user[%d] null.", pst_mac_vap->us_assoc_vap_id);

        return OAL_ERR_CODE_PTR_NULL;
    }
#endif
#ifdef _PRE_WLAN_FEATURE_11K_EXTERN
    pst_mac_user = mac_res_get_mac_user(pst_hmac_vap->pst_rrm_info->us_req_user_id);
    if (OAL_PTR_NULL == pst_mac_user)
    {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{hmac_scan_rrm_proc_save_bss::pst_mac_user[%d] null.", pst_hmac_vap->pst_rrm_info->us_req_user_id);
    ...
    MAC_GET_CB_MPDU_LEN(pst_tx_ctl)    = us_index + uc_bss_idx * (MAC_MEASUREMENT_RPT_FIX_LEN + MAC_BEACON_RPT_FIX_LEN);
    ul_ret = mac_vap_set_cb_tx_user_idx(pst_mac_vap, pst_tx_ctl, pst_mac_user->auc_user_mac_addr);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "(hmac_scan_rrm_proc_save_bss::fail to find user by xx:xx:xx:0x:0x:0x.}",
...
oal_uint32  hmac_scan_test(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_FALSE == en_2g_all && OAL_FALSE == en_5g_all)
    {
        ...
        ul_ret = mac_get_channel_idx_from_num(st_scan_req.ast_channel_list[0].en_band,
                                     st_scan_req.ast_channel_list[0].uc_chan_number,
                                     &(st_scan_req.ast_channel_list[0].uc_chan_idx));
        if (OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG1(0, OAM_SF_SCAN, "hmac_scan_test::mac_get_channel_idx_from_num fail=%d", ul_ret);
            return OAL_FAIL;
        }
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1151)
    c_obss_rssi_th = (oal_int8)hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_OBSS_RSSI_TH);
#endif
#endif
    /* 获取主信道、次信道中心频点 */
    ...
    */
    ...
    for(uc_idx = 0; uc_idx < WLAN_BAND_BUTT; uc_idx++)
    {
        if (pst_vap[uc_idx])
        {
            ...
            OAM_WARNING_LOG4(pst_vap[uc_idx]->uc_vap_id, OAM_SF_2040, "hmac_obss_init_scan_hook::before:ch=%d bw=%d, after :ch=%d bw=%d",
...
oal_void hmac_init_scan_cb(void *p_scan_record)
{
    ...
    if (OAL_PTR_NULL == pst_mac_vap)
    {
        OAM_ERROR_LOG1(0, OAM_SF_ACS, "{hmac_init_scan_cb:pst_mac_vap=NULL, vap_id=%d}", pst_scan_record->uc_vap_id);
        return;
    }
...
oal_uint32 hmac_init_scan_do(hmac_device_stru *pst_hmac_dev, mac_vap_stru *pst_mac_vap, mac_init_scan_req_stru *pst_cmd, hmac_acs_cfg_stru *pst_acs_cfg)
{
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_ACS, "{hmac_init_scan_do:get hmac vap=NULL or acs_cfg is NULL, vapid=%d}", pst_mac_vap->uc_vap_id);
        return OAL_FAIL;
    }
    ...
    for (uc_idx = 0; uc_idx < pst_cmd->ul_cmd_len; uc_idx += 2)
    {
        ...
        {
            OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ACS, "{hmac_init_scan_do:invalid channel number, ch=%d, band=%d bw=%d}",
        ...
        if (OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ACS, "{hmac_init_scan_do:mac_get_channel_idx_from_num failed en_band:[%d],chan_number:[%d]}",
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "hmac_init_scan_do::got=5:do scan mode=%d func=0x%x type=%d ch_cnt=%d\n",
                        st_scan_req.en_scan_mode,
                        st_scan_req.uc_scan_func,st_scan_req.en_scan_type, uc_cnt);
    ...
    if (uc_cnt != 0)
    {
        ...
        if(OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "hmac_init_scan_do:hmac_scan_add_req failed, ret=%d", ul_ret);
        }
    }
    else
    {
...
oal_uint32 hmac_init_scan_process(hmac_device_stru *pst_hmac_dev, mac_vap_stru *pst_mac_vap, hmac_acs_cfg_stru *pst_acs_cfg)
{
...
oal_uint32  hmac_init_scan_try(mac_device_stru *pst_mac_device, mac_vap_stru *pst_in_mac_vap, mac_try_init_scan_type en_type, hmac_acs_cfg_stru *pst_acs_cfg)
{
    oal_uint8               uc_idx;
