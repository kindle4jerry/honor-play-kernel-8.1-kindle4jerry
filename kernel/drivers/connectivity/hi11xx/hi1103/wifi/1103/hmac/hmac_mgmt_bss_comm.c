...
oal_void  hmac_rx_ba_session_decr(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA, "{hmac_rx_ba_session_decr::tid[%d] tx_session[%d] remove.}", uc_tidno, mac_mib_get_RxBASessionNumber(&pst_hmac_vap->st_vap_base_info));
}
...
oal_void  hmac_tx_ba_session_decr(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA, "{hmac_tx_ba_session_decr::tid[%d] tx_session[%d] remove.}", uc_tidno, mac_mib_get_TxBASessionNumber(&pst_hmac_vap->st_vap_base_info));
}
...
#ifdef _PRE_WLAN_1103_CHR
    hmac_chr_set_ba_info((oal_uint8)pst_action_args->ul_arg1, (oal_uint16)pst_action_args->ul_arg3);
#endif
    /* 初始化CB */
                ...
                oal_uint8                  *puc_payload)
{
    ...
    /* 判断建立能否成功 */
    uc_status = hmac_mgmt_check_set_rx_ba_ok(pst_hmac_vap, pst_hmac_user, pst_ba_rx_stru, pst_device, &(pst_hmac_user->ast_tid_info[uc_tid]));
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_req::process addba req receive and send addba rsp, uc_tid[%d], uc_status[%d], baw_size[%d], RXAMSDU[%d]}\r\n",
                ...
                oal_uint8                  *puc_payload)
{
    ...
    {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::addba rsp is received when ba status is DMAC_BA_COMPLETE or uc_dialog_token wrong.tid[%d], status[%d]}",
    ...
    if (MAC_SUCCESSFUL_STATUSCODE == uc_ba_status)
    {
        ...
        {
            hmac_tx_ba_del(pst_hmac_vap, pst_hmac_user, uc_tidno);
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::addba rsp tid[%d]，status SUCC,but token/policy wr}", uc_tidno);
            OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::rsp policy[%d],req policy[%d], rsp dialog[%d], req dialog[%d]}",
    ...
    {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::addba rsp  status err[%d].tid[%d],DEL BA.}",uc_ba_status, uc_tidno);
...
oal_uint32  hmac_mgmt_tx_addba_timeout(oal_void *p_arg)
{
    ...
    if (OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_ERROR_LOG1(0, OAM_SF_BA, "{hmac_mgmt_tx_addba_timeout::pst_hmac_user[%d] null.}", pst_alarm_data->us_mac_user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32  hmac_mgmt_reset_psm(mac_vap_stru *pst_vap, oal_uint16 us_user_id)
{
    ...
    {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_PWR, "{hmac_mgmt_reset_psm::pst_hmac_user[%d] null.}", us_user_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
                                                     ...
                                                     oal_uint16 us_init_trans_id)
{
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PMF, "{hmac_send_sa_query_req::sa_query_count %d.}", pst_hmac_user->st_sa_query_info.ul_sa_query_count);
...
oal_uint32 hmac_sa_query_interval_timeout(oal_void *p_arg)
{
    ...
    ul_passed = (oal_uint32)OAL_TIME_GET_RUNTIME(pst_interval_timer_arg->pst_hmac_user->st_sa_query_info.ul_sa_query_start_time,ul_now);

    if (ul_passed >= mac_mib_get_dot11AssociationSAQueryMaximumTimeout(pst_interval_timer_arg->pst_mac_vap))
    {
        /*change state & ul_sa_query_count=0*/
        OAM_WARNING_LOG0(pst_interval_timer_arg->pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout::Deleting user.}");
...
oal_uint32 hmac_start_sa_query(mac_vap_stru *pst_mac_vap, hmac_user_stru  *pst_hmac_user, oal_bool_enum_uint8 en_is_protected)
{
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_start_sa_query::Start a SA query process, retry_timeout %d.}",
    ul_retry_timeout);
...
oal_void  hmac_send_sa_query_rsp(mac_vap_stru *pst_mac_vap, oal_uint8 *pst_hdr, oal_bool_enum_uint8 en_is_protected)
{
    ...
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, ((mac_ieee80211_frame_stru *)pst_hdr)->auc_address2, &us_user_idx);
    if(ul_ret != OAL_SUCC)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_send_sa_query_rsp::mac_vap_find_user_by_macaddr fail,error_code=%d.}", ul_ret);
        oal_netbuf_free(pst_sa_query);
        return;
    }
...
oal_void  hmac_mgmt_send_deauth_frame(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_da, oal_uint16 us_err_code, oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "(hmac_mgmt_send_deauth_frame::fail to find user by xx:xx:xx:0x:0x:0x.}",
        puc_da[3],
        puc_da[4],
        puc_da[5]);
    }
...
oal_void  hmac_mgmt_send_disassoc_frame(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_da, oal_uint16 us_err_code, oal_bool_enum_uint8 en_is_protected)
{
    ...
    /* 增加发送去关联帧时的维测信息 */
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CONN,
                     "{hmac_mgmt_send_disassoc_frame:: DISASSOC tx, Because of err_code[%d], send disassoc frame to dest addr, da[%2X:XX:XX:XX:%2X:%2X].}",
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC, "(hmac_mgmt_send_disassoc_frame::fail to find user by xx:xx:xx:0x:0x:0x.}",
        puc_da[3],
        puc_da[4],
        puc_da[5]);
    }
                ...
                hmac_user_stru                 *pst_hmac_user)
{
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_mgmt_update_assoc_user_qos_table::pst_mac_vap null.}");
        return;
    }
    ...
    {
        mac_user_set_qos(&pst_hmac_user->st_user_base_info, pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.en_dot11QosOptionImplemented);
    }
    else
    {
...
oal_void hmac_set_user_protocol_mode(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    {
        mac_user_set_protocol_mode(pst_mac_user,WLAN_HE_MODE);
    }
    ...
    {
        mac_vap_init_by_protocol(pst_mac_vap, pst_mac_user->en_protocol_mode);
    }
}
...
oal_uint32 hmac_wpas_mgmt_tx(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    MAC_GET_CB_TX_USER_IDX(pst_tx_ctl)    = MAC_INVALID_USER_ID;                       /* 发送完成需要获取user结构体 */
...
OAL_STATIC oal_uint32  hmac_proc_location_action(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    l_str_len += OAL_SPRINTF(auc_filename + l_str_len, OAL_SIZEOF(auc_filename),"-");
    ...
    switch (pst_location_event->uc_location_type)
    {
        case MAC_HISI_LOCATION_RSSI_IE:
            ...
            l_str_len += OAL_SPRINTF(auc_filename + l_str_len, OAL_SIZEOF(auc_filename),"-RSSI.TXT");

            break;
        case MAC_HISI_LOCATION_CSI_IE:
            /* 获取文件名 MAC_ADDR_S_MAC_ADDR_C_CSI */
            l_str_len += OAL_SPRINTF(auc_filename + l_str_len, OAL_SIZEOF(auc_filename),"-CSI.TXT");

            break;
        case MAC_HISI_LOCATION_FTM_IE:
            /* 获取文件名 MAC_ADDR_S_MAC_ADDR_C_FTM */
            l_str_len += OAL_SPRINTF(auc_filename + l_str_len, OAL_SIZEOF(auc_filename),"-FTM.TXT");

            break;
        case MAC_HISI_PSD_SAVE_FILE_IE:
            /* 获取文件名 MAC_ADDR_S_MAC_ADDR_C_FTM */
            l_str_len += OAL_SPRINTF(auc_filename + l_str_len, OAL_SIZEOF(auc_filename),"-PSD.TXT");
...
oal_void  hmac_vap_set_user_avail_rates(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_vap_set_user_avail_rates::uc_avail_op_rates_num=[%d].}",st_avail_op_rates.uc_rs_nrates);
}
...
oal_uint32 hmac_proc_ht_cap_ie(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 *puc_ht_cap_ie)
{
    ...
    /* 检查空间复用节能模式 B2~B3 */
    mac_ie_proc_sm_power_save_field(pst_mac_user, (oal_uint8)((us_ht_cap_info & (BIT3 | BIT2)) >> 2));
...
oal_uint32 hmac_ie_proc_assoc_user_legacy_rate(oal_uint8 *puc_payload, oal_uint32 us_rx_len, hmac_user_stru *pst_hmac_user)
{
    ...
    /* 1.处理基础速率集 */
    puc_ie = mac_find_ie(MAC_EID_RATES, puc_payload, (oal_int32)us_rx_len);
    ...
    /* 2.处理扩展速率集 */
    puc_ie = mac_find_ie(MAC_EID_XRATES, puc_payload, (oal_int32)us_rx_len);
