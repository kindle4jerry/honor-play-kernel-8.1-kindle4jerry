...
oal_uint32  hmac_user_set_avail_num_space_stream(mac_user_stru *pst_mac_user, wlan_nss_enum_uint8 en_vap_nss)
{
    ...
    {
        en_user_num_spatial_stream = pst_mac_he_hdl->st_he_cap_ie.st_he_phy_cap.bit_dcm_max_nss_rx + 1;
    }
...
#if 0 //hmac_user_del会抛事件去执行dmac_user_del，用户已经删除，不能再抛事件去dmac操作用户
    /* 重新设置DMAC需要的参数 */
