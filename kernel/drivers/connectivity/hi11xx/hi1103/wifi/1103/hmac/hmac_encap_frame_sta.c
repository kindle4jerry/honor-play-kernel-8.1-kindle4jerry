...
oal_uint32 hmac_mgmt_encap_asoc_req_sta(hmac_vap_stru *pst_hmac_sta, oal_uint8 *puc_req_frame, oal_uint8 *puc_curr_bssid)
{
    ...
    if(OAL_PTR_NULL != pst_scaned_bss)
    {
        ...
        if((OAL_TRUE == pst_hmac_sta->bit_rrm_include_enable) || (OAL_TRUE == en_rrm_enable))
        {
    ...
    {
        mac_set_opmode_notify_ie((oal_void *)pst_mac_vap, puc_req_frame, &uc_ie_len);
        puc_req_frame += uc_ie_len;
    }
#endif
    ...
    OAM_WARNING_LOG0(0, OAM_SF_TX, "{hmac_mgmt_encap_asoc_req_sta::encap assoc req!}\r\n");
#endif
...
#ifdef _PRE_DEBUG_MODE
    OAM_WARNING_LOG0(0, OAM_SF_TX, "{hmac_mgmt_encap_auth_req::encap auth req!}\r\n");
#endif
...
#ifdef _PRE_DEBUG_MODE
    OAM_WARNING_LOG0(0, OAM_SF_TX, "{hmac_mgmt_encap_auth_req_seq3::encap auth req seq3!}\r\n");
#endif
