...
oal_void hmac_rx_ddc_switch(oal_uint32 ul_rx_throughput_mbps)
{
    ...
    oal_uint32      ul_ret;
    oal_bool_enum_uint8   en_ddc;
    /* 如果定制化不支持硬件聚合 */
    if (OAL_FALSE == g_st_rx_ddc_switch.uc_ini_ddc_en)
    {
        return;
    }
    /* 每秒吞吐量门限 */
    if ((0 != g_st_rx_ddc_switch.us_ddc_rx_throughput_high) && (0 != g_st_rx_ddc_switch.us_ddc_rx_throughput_low))
    {
        ul_limit_throughput_high = g_st_rx_ddc_switch.us_ddc_rx_throughput_high;
        ul_limit_throughput_low  = g_st_rx_ddc_switch.us_ddc_rx_throughput_low;
    }
    else
    {
        ul_limit_throughput_high = WLAN_DDC_THROUGHPUT_THRESHOLD_HIGH;
        ul_limit_throughput_low  = WLAN_DDC_THROUGHPUT_THRESHOLD_LOW;
    }
    ...
    /* 当前方式相同,不处理 */
    if (g_st_rx_ddc_switch.uc_cur_ddc_status == en_ddc)
    {
        return;
    }
    ...
    ul_ret = hmac_config_send_event(pst_mac_vap, WLAN_CFGID_DDC_SWITCH, OAL_SIZEOF(oal_uint8), (oal_uint8 *)(&en_ddc));
    g_st_rx_ddc_switch.uc_cur_ddc_status = en_ddc;

    OAM_WARNING_LOG3(0,OAM_SF_ANY,"{hmac_rx_ddc_switch: limit_high = [%d],limit_low = [%d],tx_throught= [%d]!}",
...
oal_void  hmac_tx_ampdu_switch(oal_uint32 ul_tx_throughput_mbps)
{
    ...
    hmac_config_set_ampdu_tx_on(pst_mac_vap, OAL_SIZEOF(mac_cfg_ampdu_tx_on_param_stru), (oal_uint8*)&st_ampdu_tx_on);
}
#endif
                ...
                mac_tx_ctl_stru   *pst_tx_ctl)
{
   ...
   /*安全过滤*/
    ...
    if(OAL_TRUE == pst_hmac_dev->en_car_enable_flag)
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG1(0, 0, "hmac_tx_ucast_process:hmac_car_process: downlink car: DROP PACKET! ul_ret[%d]", ul_ret);
...
/*lint -e695*/
oal_uint32  hmac_tx_encap(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)/*lint !e695*/
{
    ...
    else
    {
        ...
        if (MAC_GET_CB_HAS_EHTER_HEAD(pst_tx_ctl))
        {
            /* 恢复data指针到ETHER HEAD,+2保证4bytes对齐 */
            oal_netbuf_push(pst_buf, ETHER_HDR_LEN + 2);
        }
#endif
    }
    ...
    if (0 != ul_threshold)
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
            OAM_ERROR_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_tx_encap::hmac_en_mic failed[%d].}", ul_ret);
            return ul_ret;
        }
                ...
                mac_tx_ctl_stru    *pst_tx_ctl)
{
    ...
    else /* 组播 or 广播 */
    {
        ...
        #ifdef _PRE_WLAN_FEATURE_CAR
            /* 未知组播，car下行丢包处理 */
            pst_hmac_dev = hmac_res_get_mac_dev(pst_vap->st_vap_base_info.uc_device_id);
...
OAL_INLINE oal_uint32  hmac_tx_lan_to_wlan_no_tcp_opt(mac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
{
    ...
    {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_TX, "{hmac_tx_lan_to_wlan_no_tcp_opt::en_vap_mode=%d.}", pst_vap->en_vap_mode);
        OAM_STAT_VAP_INCR(pst_vap->uc_vap_id, tx_abnormal_msdu_dropped, 1);
    ...
    {
        OAM_WARNING_LOG2(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_tx_lan_to_wlan_no_tcp_opt::send datatype==%u, len==%u}[0:dhcp 1:eapol 2:arp_rsp 3:arp_req]", uc_data_type, OAL_NETBUF_LEN(pst_buf));
    }
    ...
    {
        if (OAL_TRUE == wlan_check_arp_spoofing((struct net_device*)(hmac_vap_get_net_device(pst_vap->uc_vap_id)), (struct sk_buff*)pst_buf))
        {
            OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_tx_lan_to_wlan_no_tcp_opt::ARP spoofing, type is %d", uc_data_type);
            oam_report_eth_frame(pst_vap->auc_bssid,
    ...
    else if (WLAN_VAP_MODE_BSS_STA == pst_hmac_vap->st_vap_base_info.en_vap_mode)
    {
            ...
            //&& OAL_UNLIKELY(WAPI_IS_WORK(pst_hmac_vap)))
        {
            ...
            {
                OAM_STAT_VAP_INCR(pst_vap->uc_vap_id, tx_abnormal_msdu_dropped, 1);
                OAM_WARNING_LOG1(0, OAM_SF_ANY, "hmac_tx_lan_to_wlan_no_tcp_opt::hmac_user_get_wapi_ptr fail! us_assoc_id[%u]}", pst_vap->us_assoc_vap_id);
...
oal_net_dev_tx_enum  hmac_bridge_vap_xmit(oal_netbuf_stru *pst_buf, oal_net_device_stru *pst_dev)
{
    ...
    if ( PROXYSTA_MODE_SSTA == pst_hmac_vap->en_proxysta_mode )
    {
        ...
        pst_hmac_user = mac_res_get_hmac_user(pst_vap->us_assoc_vap_id);
        if (OAL_PTR_NULL == pst_hmac_user)
        {
            OAM_ERROR_LOG0(pst_vap->uc_vap_id, OAM_SF_CFG, "{hmac_bridge_vap_xmit::pst_hmac_user null.}");
            oal_netbuf_free(pst_buf);
            oal_spin_unlock_bh(&pst_hmac_vap->st_lock_state);
            return OAL_NETDEV_TX_OK;
        }
...
oal_net_dev_tx_enum  hmac_vap_start_xmit(oal_netbuf_stru *pst_buf, oal_net_device_stru *pst_dev)
{
   ...
   OAM_PROFILING_TX_STATISTIC(OAL_PTR_NULL, OAM_PROFILING_FUNC_CONFIG_XMIT_START);
    ...
    if( PKT_TRACE_BUTT != en_trace_pkt_type)
    {
        ...
        oam_report_eth_frame(auc_user_macaddr,
                                oal_netbuf_data(pst_buf),
                                (oal_uint16)OAL_NETBUF_LEN(pst_buf),
                                OAM_OTA_FRAME_DIRECTION_TYPE_TX);
    }
    ...
    uc_data_type = mac_get_data_type_from_8023(puc_frame_hdr, MAC_NETBUFF_PAYLOAD_ETH);
#ifdef _PRE_WLAN_FEATURE_DHCP_REQ_DISABLE
