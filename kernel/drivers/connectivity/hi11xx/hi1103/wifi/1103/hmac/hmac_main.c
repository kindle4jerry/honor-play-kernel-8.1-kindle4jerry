...
oal_uint32  hmac_init_scan_do(hmac_device_stru *pst_mac_dev, mac_vap_stru *pst_vap, mac_init_scan_req_stru *pst_cmd, hmac_acs_cfg_stru *pst_acs_cfg);
 ...
 oal_uint32 hmac_init_event_process(frw_event_mem_stru *pst_event_mem)
{
    ...
    ul_return = hmac_board_init(ul_chip_max_num);
...
OAL_STATIC oal_void hmac_event_fsm_tx_adapt_subtable_register(oal_void)
{
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_STA_SET_MU_EDCA_REG].p_tx_adapt_func = hmac_proc_set_mu_edca_param_tx_adapt;
#endif
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_CALI_HMAC2DMAC].p_tx_adapt_func = hmac_send_event_netbuf_tx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_CALI_MATRIX_HMAC2DMAC].p_tx_adapt_func  = hmac_send_event_netbuf_tx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_APP_IE_H2D].p_tx_adapt_func     = hmac_send_event_netbuf_tx_adapt;
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_APF_CMD].p_tx_adapt_func = hmac_send_event_netbuf_tx_adapt;
#endif
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WALN_CTX_EVENT_SUB_TYPR_DFS_CAC_CTRL_TX].p_tx_adapt_func = hmac_hcc_tx_convert_event_to_netbuf_uint16;
#endif
    ...
    g_ast_dmac_host_crx_table[HMAC_TO_DMAC_AUTOCALI_CMD].p_tx_adapt_func = hmac_sdt_recv_sample_cmd_tx_adapt;
#endif
...
OAL_STATIC oal_void hmac_event_fsm_rx_adapt_subtable_register(oal_void)
{
    ...
    g_ast_hmac_wlan_drx_event_sub_table[DMAC_WLAN_DRX_EVENT_SUB_TYPE_RX_DATA].p_rx_adapt_func = hmac_rx_process_data_rx_adapt;
...
#ifdef _PRE_WLAN_RF_AUTOCALI
    g_ast_hmac_wlan_ctx_event_sub_table[DMAC_TO_HMAC_AUTOCALI_DATA].p_rx_adapt_func = hmac_cali2hmac_misc_event_rx_adapt;
#endif
}
...
OAL_STATIC oal_void hmac_event_fsm_action_subtable_register(oal_void)
{
    ...
    /* 注册HMAC模块WLAN_DRX事件子表 */
    g_ast_hmac_wlan_drx_event_sub_table[DMAC_WLAN_DRX_EVENT_SUB_TYPE_RX_DATA].p_func = hmac_rx_process_data_event;
...
#endif
#ifdef _PRE_WLAN_RF_AUTOCALI
    g_ast_hmac_wlan_ctx_event_sub_table[DMAC_TO_HMAC_AUTOCALI_DATA].p_func = hmac_sdt_send_autocali_data;
#endif
...
oal_bool_enum_uint8 hmac_get_pm_pause_func(oal_void)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_hmac_device->pst_device_base_info->uc_vap_num; uc_vap_idx++)
    {
        ...
        if (OAL_TRUE == pst_hmac_vap->st_ps_sw_timer.en_is_registerd)
        {
            ...
            {
                OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG, "{hmac_get_pm_pause_func::g_uc_check_timer_cnt[%d]", pst_hmac_vap->us_check_timer_pause_cnt);
            }

            if (pst_hmac_vap->us_check_timer_pause_cnt > HMAC_SWITCH_STA_PSM_MAX_CNT)
            {
                uc_is_any_cnt_exceed_limit = OAL_TRUE;
                OAM_ERROR_LOG2(0, OAM_SF_CFG, "{hmac_get_pm_pause_func::sw ps timer cnt too large[%d]> max[%d]}", pst_hmac_vap->us_check_timer_pause_cnt, HMAC_SWITCH_STA_PSM_MAX_CNT);
            }
        }
        else
        {
            if (pst_hmac_vap->us_check_timer_pause_cnt != 0)
            {
                ...
                OAM_WARNING_LOG2(0, OAM_SF_CFG, "{hmac_get_pm_pause_func::g_uc_check_timer_cnt end[%d],max[%d]", pst_hmac_vap->us_check_timer_pause_cnt, HMAC_SWITCH_STA_PSM_MAX_CNT);
            }
...
oal_void hmac_rxdata_netbuf_enqueue(oal_netbuf_stru  *pst_netbuf)
{
    ...
    oal_spin_lock(&g_st_rxdata_thread.st_lock);
    oal_netbuf_list_tail_nolock(&g_st_rxdata_thread.st_rxdata_netbuf_head, pst_netbuf);
    oal_spin_unlock(&g_st_rxdata_thread.st_lock);
}
...
OAL_STATIC oal_int32 hmac_rxdata_thread(oal_void* p_data)
{
...
#endif
    {
        ...
        oal_spin_lock(&g_st_rxdata_thread.st_lock);
        pst_netbuf = oal_netbuf_delist_nolock(&g_st_rxdata_thread.st_rxdata_netbuf_head);
        oal_spin_unlock(&g_st_rxdata_thread.st_lock);
        if(pst_netbuf)
        {
...
oal_uint32  hmac_sdt_send_autocali_data(frw_event_mem_stru  *pst_event_mem)
{
    ...
    pst_net_buf_repot = oam_alloc_data2sdt(OAL_NETBUF_LEN(pst_autocali_event->pst_netbuf));
    if (!pst_net_buf_repot)
    {
        OAL_IO_PRINT("hmac_sdt_autocali_data::oam_alloc_data2sdt return null, us_payload_len[%d].\r\n", OAL_NETBUF_LEN(pst_autocali_event->pst_netbuf));
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_config_set_acs_cmd(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if(pst_acs_cmd->uc_cmd == DMAC_ACS_CMD_DO_SCAN)
    {
        ...
        return hmac_init_scan_do(pst_hmac_device, pst_mac_vap, (mac_init_scan_req_stru *)pst_acs_cmd, &st_acs_cfg);
    }
