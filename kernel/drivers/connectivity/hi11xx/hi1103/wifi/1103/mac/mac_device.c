...
oal_uint32  mac_board_init(void)
{
    ...
    {
        OAL_IO_PRINT("[file = %s, line = %d], mac_board_init, memory allocation g_pst_blacklist %u bytes fail!\n",
...
oal_uint32  mac_device_init(mac_device_stru *pst_mac_device, oal_uint32 ul_chip_ver, oal_uint8 uc_chip_id, oal_uint8 uc_device_id)
{
    ...
    {
        OAM_ERROR_LOG3(0, OAM_SF_ANY, "{mac_device_init::pst_mac_device device id[%d] chip id[%d] >support[%d].}",uc_device_id, uc_chip_id, WLAN_SERVICE_DEVICE_MAX_NUM_PER_CHIP);
   ...
   switch(ul_chip_ver)
   {
    ...
    {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{mac_device_init::alloc fcs_cfg failed, abort.}");
        return OAL_FAIL;
    }
#endif
