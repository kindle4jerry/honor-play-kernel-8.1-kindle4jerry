...
#else
    ul_ret = oam_report_beacon((oal_uint8 *)MAC_GET_RX_CB_MAC_HEADER_ADDR(pst_rx_cb),
                               pst_rx_cb->uc_mac_header_len,
                                           ...
                                           oam_ota_type_enum_uint8 en_ota_type)
{
    ...
    if ((OAM_OTA_TYPE_RX_DSCR == en_ota_type)||(OAM_OTA_TYPE_RX_DSCR_PILOT== en_ota_type))
    {
        ...
        {
            ul_ret = oam_report_dscr(auc_user_macaddr, puc_des_addr,(oal_uint16)WLAN_RX_DSCR_SIZE, en_ota_type);
            if (OAL_SUCC != ul_ret)
            {
...
oal_void mac_set_dsss_params(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len,oal_uint8 uc_chan_num)
{
    /***************************************************************************
                        ----------------------------------------
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC != _PRE_MULTI_CORE_MODE)
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    if(OAL_TRUE != hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_BEACON_COUNTRY_IE_SWITCH))
    {
        *puc_ie_len = 0;
        return;
    }
...
oal_void mac_set_rsn_ie(oal_void *pst_vap, oal_uint8 *puc_pmkid, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    mac_vap_stru     *pst_mac_vap = (mac_vap_stru *)pst_vap;
...
oal_void mac_set_wpa_ie(oal_void  *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    mac_vap_stru     *pst_mac_vap = (mac_vap_stru *)pst_vap;
    oal_uint8         uc_index;
    oal_uint8         uc_pair_suites_num;
...
oal_void  mac_set_wmm_ac_params(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_buffer, wlan_wme_ac_type_enum_uint8 en_ac)
{
    ...
    /* ACM */
#if 0 // TBD:APUT WMMAC
    if ((OAL_TRUE == g_en_wmmac_switch) && ((WLAN_WME_AC_VO == en_ac) || (WLAN_WME_AC_VI == en_ac)))
    {
...
oal_void mac_set_power_cap_ie(oal_uint8 *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    if (OAL_FALSE == mac_mib_get_dot11SpectrumManagementRequired(pst_mac_vap)
        && OAL_FALSE == mac_mib_get_dot11RadioMeasurementActivated(pst_mac_vap))
    {
        OAM_WARNING_LOG0(0, OAM_SF_TPC, "{mac_set_power_cap_ie::en_dot11SpectrumManagementRequired and en_dot11RadioMeasurementActivated are FALSE!}");
        *puc_ie_len = 0;
        return;
    }
...
oal_void mac_set_opmode_field(oal_uint8 *pst_vap, oal_uint8 *puc_buffer, wlan_nss_enum_uint8 en_nss)
{
    mac_vap_stru           *pst_mac_vap       = (mac_vap_stru *)pst_vap;
...
oal_void mac_set_opmode_notify_ie(oal_uint8 *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    ********************************************/
    ...
    mac_set_opmode_field((oal_void*)pst_vap, (puc_buffer + MAC_IE_HDR_LEN), pst_mac_vap->en_vap_rx_nss);
