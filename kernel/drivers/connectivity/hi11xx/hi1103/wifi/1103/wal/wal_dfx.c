...
OAL_STATIC oal_uint32  wal_dfr_recovery_env(void)
{
    ...
    CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_WIFI_DRV(CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_RECV_LASTWORD));
    ...
    OAM_WARNING_LOG2(0, OAM_SF_ANY, "wal_dfr_recovery_env: get plat_process_comp signal after[%u]ms, netdev_num=%d!",
        (oal_uint32)OAL_JIFFIES_TO_MSECS(OAL_MSECS_TO_JIFFIES(DFR_WAIT_PLAT_FINISH_TIME) - ul_timeleft), g_st_dfr_info.ul_netdev_num);

    /* 恢复vap, 上报异常给上层 */
    for (; g_st_dfr_info.ul_netdev_num > 0; g_st_dfr_info.ul_netdev_num--)
    {
        ul_ret = OAL_SUCC;
        pst_netdev = g_st_dfr_info.past_netdev[g_st_dfr_info.ul_netdev_num - 1];
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)&&(_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    l_plat_ret = plat_exception_handler(SUBSYS_WIFI, THREAD_WIFI, ul_exception_type);
#endif
...
oal_uint32 wal_dfr_excp_rx(oal_uint8 uc_device_id, oal_uint32 ul_exception_type)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_dev->uc_vap_num; uc_vap_idx++)
    {
        ...
        if (OAL_PTR_NULL == pst_hmac_vap)
        {
            OAM_WARNING_LOG2(0, OAM_SF_DFR, "{wal_dfr_excp_rx::mac_res_get_hmac_vap fail!vap_idx = %u dev_ID[%d]}\r",pst_mac_dev->auc_vap_id[uc_vap_idx], uc_device_id);
            continue;
        }
