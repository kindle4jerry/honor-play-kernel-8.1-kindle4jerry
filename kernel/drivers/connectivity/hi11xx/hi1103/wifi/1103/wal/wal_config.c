...
OAL_STATIC oal_uint32  wal_config_set_mcsax(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_config_set_mcsax::hmac_config_set_mcsac error!}\r\n");
        return ul_ret;
    }
...
oal_uint32 wal_autocali_report2sdt(frw_event_mem_stru *pst_event_mem)
{
    ...
    oam_report_data2sdt(*(oal_netbuf_stru**)pst_event->auc_event_data, OAM_DATA_TYPE_AUTOCALI , OAM_PRIMID_TYPE_DEV_ACK);
    return OAL_SUCC;
}
#endif
...
#endif
#ifdef _PRE_WLAN_RF_AUTOCALI
    g_st_wal_drv_func_hook.p_wal_recv_autocali_data_func  = wal_recv_autocali_cmd;
#endif
...
oal_uint32 wal_send_cali_data(oal_net_device_stru *pst_net_dev)
{
    ...
    if (pst_mac_vap != OAL_PTR_NULL)
    {
        hmac_send_cali_data(pst_mac_vap);
    }
    else
    {
        OAL_IO_PRINT("wal_send_cali_data:netdev name %s not have dev priv-vap", pst_net_dev->name);
        OAM_WARNING_LOG1(0, OAM_SF_CALIBRATE, "wal_send_cali_data:netdev[%p] have no dev priv-vap", pst_net_dev);
