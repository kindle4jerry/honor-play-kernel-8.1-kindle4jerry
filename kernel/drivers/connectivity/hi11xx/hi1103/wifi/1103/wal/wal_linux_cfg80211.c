...
oal_uint32 wal_find_wmm_uapsd(oal_uint8 *puc_frame_body, oal_int32 l_len)
{
    ...
    while (l_index < l_len)
    {
        if ((MAC_EID_WMM == puc_frame_body[l_index])
            && (0 == oal_memcmp(puc_frame_body + l_index + 2, g_auc_wmm_oui, MAC_OUI_LEN))
...
oal_void wal_parse_wpa_wpa2_ie(oal_beacon_parameters *pst_beacon_info,
                                 mac_beacon_param_stru *pst_beacon_param)
{
    ...
    if (WLAN_WITP_CAPABILITY_PRIVACY & us_capability_info)
    {
        ...
        puc_ie_tmp = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, pst_beacon_info->tail, pst_beacon_info->tail_len);
        if (OAL_PTR_NULL != puc_ie_tmp)
        {
                                 ...
                                 mac_beacon_param_stru *pst_beacon_param)
{
    ...
    /* 查找 MD 信息元素 */
    puc_ie_tmp = mac_find_ie(MAC_EID_MOBILITY_DOMAIN, pst_beacon_info->tail, pst_beacon_info->tail_len);
    if (OAL_PTR_NULL != puc_ie_tmp)
    {
...
OAL_STATIC oal_void wal_set_crypto_info(mac_conn_param_stru *pst_conn_param, oal_cfg80211_conn_stru *pst_sme)
{
    ...
    if (pst_sme->privacy)
    {
        if (0 != pst_sme->key_len)
        {
            /* 设置wep加密信息 */
            pst_conn_param->puc_wep_key               = pst_sme->key;
        ...
        /* 优先查找 RSN 信息元素 */
        puc_ie_tmp = mac_find_ie(MAC_EID_RSN, (oal_uint8 *)pst_sme->ie, (oal_int32)pst_sme->ie_len);
        if (OAL_PTR_NULL != puc_ie_tmp)
        {
        ...
        {
            /* WPA */
            puc_ie_tmp = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, (oal_uint8 *)pst_sme->ie, (oal_int32)pst_sme->ie_len);
            if (OAL_PTR_NULL != puc_ie_tmp)
            {
...
OAL_STATIC oal_int32  wal_cfg80211_connect(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_net_device, oal_cfg80211_conn_stru *pst_sme)
{
    ...
    /* 解析内核下发的 ssid */
    st_mac_conn_param.puc_ssid      = (oal_uint8 *)pst_sme->ssid;
    st_mac_conn_param.uc_ssid_len   = (oal_uint8)pst_sme->ssid_len;

#if (defined(_PRE_PRODUCT_ID_HI110X_HOST))
    if (pst_sme->bssid)
    {
        st_mac_conn_param.puc_bssid = (oal_uint8 *)pst_sme->bssid;
    }
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,16,0))
    else if (pst_sme->bssid_hint)
    {
        st_mac_conn_param.puc_bssid = (oal_uint8 *)pst_sme->bssid_hint;
    }
#endif
    else
    {
    ...
    /* 设置关联P2P/WPS ie */
    st_mac_conn_param.puc_ie        = (oal_uint8 *)pst_sme->ie;
                                            ...
                                            oal_uint8                uc_key_index)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_wiphy) || OAL_UNLIKELY(OAL_PTR_NULL == pst_netdev))
    {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "{wal_cfg80211_set_default_mgmt_key::pst_wiphy or pst_netdev ptr is null,error %d, %d!}\r\n",
                       pst_wiphy, pst_netdev);
        return -OAL_EINVAL;
    }
                ...
                oal_beacon_parameters *pst_beacon_info)
{
    ...
    *****************************************************************************/
    ...
    wal_parse_wpa_wpa2_ie(pst_beacon_info, &st_beacon_param);
                ...
                oal_beacon_parameters *pst_beacon_info)
{
    ...
    *****************************************************************************/
    ...
    /* 5.启动AP */
    #if defined(_PRE_PRODUCT_ID_HI110X_HOST)
    l_ret = wal_start_vap(pst_dev);
#else
    l_ret = wal_netdev_open(pst_dev,OAL_FALSE);
#endif
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
                                                           ...
                                                           mac_beacon_param_stru       *pst_beacon_param)
{
    ...
    *****************************************************************************/
    ...
    /* 配置Wmm信息元素 */
    ul_ret = wal_parse_wmm_ie(pst_netdev, pst_mac_vap, &st_beacon_info_tmp);
    if (OAL_SUCC != ul_ret)
    {
                                                          ...
                                                          oal_net_device_stru *pst_netdev)
{
    ...
    ***************************************************************************/
    ...
    if(OAL_SUCC != ul_err_code)
    {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_set_channel_info::wal_check_and_release_msg_resp fail return err code: [%u].}",
                       ul_err_code);
        return -OAL_EFAIL;
    }
...
#endif
{
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_CFG, "{wal_cfg80211_change_virtual_intf::mac_res_get_dev fail device id[%d].}",pst_mac_vap->uc_device_id);
        return -OAL_EINVAL;
    }
                                                   ...
                                                   oal_station_info_stru *pst_sta_info)
{
    ...
    ********************************************************************************/
    ...
    else
    {
        ...
        wal_cfg80211_fill_station_info(pst_sta_info, &pst_hmac_vap->station_info);
        /*lint -e571*/
        //OAM_WARNING_LOG3(0, OAM_SF_CFG, "{wal_cfg80211_get_station::rssi [%d], snr_ant0 [%d/2], snr_ant1 [%d/2].}",
        //        pst_hmac_vap->station_info.signal, pst_hmac_vap->st_station_info_extend.c_snr_ant0, pst_hmac_vap->st_station_info_extend.c_snr_ant1);
        /*lint +e571*/
        return OAL_SUCC;
    }
}
                                                    ...
                                                    wlan_ieee80211_roc_type_uint8 en_roc_type)
{
    ...
    ***************************************************************************/
    ...
    if (OAL_SUCC != l_ret)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_P2P, "{wal_drv_remain_on_channel::wal_send_cfg_event return err code:[%d]!}\r\n",l_ret);
        return -OAL_EFAIL;
    }
    ...
    if(OAL_SUCC != ul_err_code)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_drv_remain_on_channel::wal_send_cfg_event return err code:[%u]!}\r\n",
                         ul_err_code);
        return -OAL_EFAIL;
    }
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++;   /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    switch (pst_hmac_vap->st_vap_base_info.en_vap_mode)
    {
        case WLAN_VAP_MODE_BSS_AP:
            OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_mgmt_tx::p2p mode[%d]tx mgmt vap channel[%d]mgmt tx channel[%d]",
                                pst_hmac_vap->st_vap_base_info.en_p2p_mode, pst_hmac_vap->st_vap_base_info.st_channel.uc_chan_idx, st_mgmt_tx.channel);
            if ((pst_hmac_vap->st_vap_base_info.st_channel.uc_chan_idx != st_mgmt_tx.channel) && IS_P2P_GO(&pst_hmac_vap->st_vap_base_info))
            {
    ...
    if ((OAL_TRUE == en_need_offchan) && (0 == ul_wait))
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_mgmt_tx::need offchannel,but wait time is 0}");
    ...
    do
    {
        ul_ret = wal_mgmt_do_tx(pst_netdev, &st_mgmt_tx, en_offchan);
        uc_retry++;
    }while((ul_ret != DMAC_TX_SUCC) && (ul_ret != DMAC_TX_INVALID) && (uc_retry <= WAL_MGMT_TX_RETRY_CNT) && time_before(OAL_TIME_JIFFY, ul_start_time_stamp + OAL_MSECS_TO_JIFFIES(ul_wait)));/*lint !e666*/
...
oal_void wal_cfg80211_reset_bands(oal_uint8  uc_dev_id)
{
    int i;
...
oal_void wal_cfg80211_save_bands(oal_uint8  uc_dev_id)
{
    int i;
...
oal_uint32  wal_cfg80211_init(oal_void)
{
    ...
    for (ul_chip = 0; ul_chip < ul_chip_max_num; ul_chip++)
    {
        for (uc_device = 0; uc_device < pst_hmac_board->ast_chip[ul_chip].uc_device_nums; uc_device++)
        {
            ...
            if (OAL_PTR_NULL == pst_cfg_hmac_vap)
            {
                OAM_ERROR_LOG1(0, OAM_SF_CFG, "{wal_cfg80211_init::pst_cfg_hmac_vap is null vap_id:%d!}\r\n",pst_device->uc_cfg_vap_id);
                OAL_IO_PRINT("wal_cfg80211_init::Get cfg vap Hisilicon[%d] failed, cfg vap id is %d\n", uc_dev_id, pst_device->uc_cfg_vap_id);
                oal_wiphy_free(pst_device->pst_wiphy);
                return OAL_FAIL;
            }
            ...
            if (OAL_PTR_NULL == pst_net_dev)
            {
                OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_init::pst_net_dev of cfg vap is null ptr!}\r\n");
                OAL_IO_PRINT("wal_cfg80211_init::Get cfg vap Hisilicon[%d] net dev failed, cfg vap id is %d\n", uc_dev_id, pst_device->uc_cfg_vap_id);
                oal_wiphy_free(pst_device->pst_wiphy);
                return OAL_FAIL;
            }
            ...
            if(OAL_SUCC != l_return)
            {
                OAL_IO_PRINT("wal_init_wlan_netdev hwlan[%d] failed.l_return:%d\n",uc_dev_id,l_return);
...
OAL_STATIC oal_int32 wal_cfg2io_set_channel_info(oal_net_device_stru *pst_netdev, mac_cfg_channel_param_stru *pst_chan)
{
    ...
    ***************************************************************************/
    ...
    if(OAL_SUCC != l_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_CHAN, "{wal_cfg2io_set_channel_info::wal_send_cfg_event return err code: [%d]!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if(OAL_SUCC != ul_err_code)
    {
        OAM_ERROR_LOG1(0, OAM_SF_CHAN, "{wal_cfg2io_set_channel_info::wal_send_cfg_event return err code: [%u].}",
                       ul_err_code);
        return -OAL_EFAIL;
    }
...
oal_uint32 wal_cfg80211_m2s_status_report(frw_event_mem_stru *pst_event_mem)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_M2S, "{wal_cfg80211_m2s_status_report::vap num[%d]device result[%d]state[%d]m2s_mode[%d].}",
        pst_m2s_switch_comp_status->uc_vap_num, pst_m2s_switch_comp_status->en_m2s_result,
        pst_m2s_switch_comp_status->uc_m2s_state, pst_m2s_switch_comp_status->uc_m2s_mode);

    for (uc_vap_idx = 0; uc_vap_idx < pst_m2s_switch_comp_status->uc_vap_num; uc_vap_idx++)
    {
