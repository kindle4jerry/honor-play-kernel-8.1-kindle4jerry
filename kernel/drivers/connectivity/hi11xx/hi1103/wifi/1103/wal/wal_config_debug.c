...
OAL_STATIC oal_uint32  wal_config_packet_xmit(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    else
    {
        for (uc_skb_idx = 0; uc_skb_idx < uc_skb_num; uc_skb_idx++)
        {
            ...
            {
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_config_create_packet_ap:: alloc netbuf failed, skb_idx: %d!}\r\n", uc_skb_idx);
                continue;
            }
#ifdef _PRE_WLAN_FEATURE_SMP_SUPPORT
...
OAL_STATIC oal_uint32  wal_config_alg_cfg(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    return wal_hipriv_alg_cfg(pst_net_device, (oal_int8 *)puc_param);
}
...
{
    return hmac_config_sync_cmd_common(pst_mac_vap,WLAN_CFGID_FREE_RATIO_SET,us_len, puc_param);
}
#endif
