...
oal_uint32 hwifi_cfg_host_global_init_param(oal_void)
{
    ...
    g_wlan_fast_check_cnt   = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_FAST_CHECK_CNT);

    g_wlan_pm_switch = (g_wlan_device_pm_switch == 1 || g_wlan_device_pm_switch == 4)  ?  OAL_TRUE : OAL_FALSE;
    ...
    if (OAL_SUCC == l_ret)
    {
        g_us_download_rate_limit_pps = (oal_uint16)l_priv_value;
    }
#endif
    return OAL_SUCC;
}
...
oal_int32  host_module_init(oal_void)
{
    ...
    if (OAL_SUCC == l_return)
    {
        ...
        OAL_IO_PRINT("host_module_init:en_cali_first_power_on pri_val[0x%x]first_pow[%d]\r\n", l_priv_value, en_cali_first_power_on);
    }
    #endif
...
oal_int32  hi110x_host_main_init(oal_void)
{
    ...
    if (OAL_SUCC != l_return)
    {
        OAL_IO_PRINT("hi110x_host_main_init: host_module_init return error code: %d", l_return);
        return l_return;
    }
