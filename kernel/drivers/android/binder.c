	...
	struct binder_proc *proc, struct binder_thread *thread)
{
	...
	if (buffer->target_node)
		...
		binder_enqueue_work(thread->proc, &thread->reply_error.work,
			&thread->todo);
	}
				...
				u64 timestamp)
{
	...
	spin_lock(&t->lock);
	to_proc = t->to_proc;
	from_pid = t->from ? t->from->proc->pid : (t->async_from ? t->async_from->proc->pid : 0);
	to_pid = to_proc ? to_proc->pid : 0;

	seq_printf(m,
		   "%s%d:%d to %d:%d code %x wait:%lld.%lld s\n",
		   prefix,
		   from_pid, t->from ? t->from->pid : (t->async_from ? t->async_from->pid : 0),
