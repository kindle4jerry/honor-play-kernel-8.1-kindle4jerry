...
static long get_phys_addr(struct fd_device *pdev)
{
    struct ion_handle *ionhnd;
    ionhnd = ion_import_dma_buf(pdev->ion_client, pdev->ipu_share_fd);/*lint !e838*/
    if (IS_ERR(ionhnd)) {
        fd_err("%s:invalid ion handle ipu share fd", __func__);
        return -1;
    } else {
        if (0 != ion_phys(pdev->ion_client, ionhnd, &pdev->mem_base.ipu_phys_addr, &pdev->mem_base.ipu_size)) {
            fd_err("%s, ion_phys error(ipu)", __func__);
            ion_free(pdev->ion_client, ionhnd);
            return -1;
        }
    }
    ion_free(pdev->ion_client, ionhnd);
    ionhnd = ion_import_dma_buf(pdev->ion_client, pdev->img_share_fd);/*lint !e838*/
    if (IS_ERR(ionhnd)) {
        fd_err("%s:invalid ion handle img share fd", __func__);
        return -1;
    } else {
        if (0 != ion_phys(pdev->ion_client, ionhnd, &pdev->mem_base.img_phys_addr, &pdev->mem_base.img_size)) {
            fd_err("%s, ion_phys error(img)", __func__);
            ion_free(pdev->ion_client, ionhnd);
            return -1;
        }
    }
...
static int fd_write_reg(struct fd_device *pdev, uint32_t * regStart)
{
    ...
    for (k = 0; k < size; k++) {
        reg_val = *(regStart++);
        if ((0 == pdev->smmu_flag) && (((FD_REGISTER_IPP_CTRL == offset) && (0 != (reg_val & 4)))
                || ((FD_REGISTER_TME_BLK_EN == offset) && (0 != reg_val)))) {
