...
struct mutex imx498imx550hybird_par_power_lock;
...
static int imx498imx550hybird_config_power_on(hwsensor_intf_t* si)
{
    int ret = 0;
    mutex_lock(&imx498imx550hybird_par_power_lock);

    if (NULL == si || NULL == si->vtbl || NULL == si->vtbl->power_up)
    {
        cam_err("%s. si,si->vtbl or si->vtbl->power_up is null", __func__);
        /*lint -e455 -esym(455,*)*/
        mutex_unlock(&imx498imx550hybird_par_power_lock);
        /*lint -e455 +esym(455,*)*/
        return -EINVAL;
    }
...
static int imx498imx550hybird_config_power_off(hwsensor_intf_t* si)
{
    int ret = 0;
    mutex_lock(&imx498imx550hybird_par_power_lock);
...
static int imx498imx550hybird_config_match_id(hwsensor_intf_t* si, void *argp)
{
    int ret = 0;
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        case SEN_CONFIG_POWER_ON:
            ret = imx498imx550hybird_config_power_on(si);
            break;
        case SEN_CONFIG_POWER_OFF:
            ret = imx498imx550hybird_config_power_off(si);
            break;
        ...
        case SEN_CONFIG_MATCH_ID:
            ret = imx498imx550hybird_config_match_id(si, argp);
            break;
...
{
    {
        .compatible = "huawei,imx498imx550hybird_par",
...
s_imx498imx550hybird_driver =
{
    ...
    .driver =
    {
        .name = "huawei,imx498imx550hybird_par",
        .owner = THIS_MODULE,
        ...
        struct platform_device* pdev)
{
    ...
    mutex_init(&imx498imx550hybird_par_power_lock);
    np = pdev->dev.of_node;
    if (NULL == np) {
...
MODULE_DESCRIPTION("imx498imx550hybird_par");
