...
acm_static void acm_dmd_add(struct list_head *head,acm_list_node_t *list_node) {
	...
	if (acm_dmd_list.nr_nodes > ACM_DMD_MAX_NODES - 1) {
		printk("ACM:List was too long!Data will be dropped!Pkgname=%s Pic_path=%s\n",
			list_node->pkgname,list_node->pic_path);
...
static int do_cmd_search(unsigned long args) {
	...
	printk("ACM:Search result:pkgname=%s pic_path=%s flag=%d\n",temp_mp_node.pkgname,
		temp_mp_node.pic_path,temp_mp_node.flag);
...
acm_static void acm_fwk_upload(void) {
	...
	while (1) {
		...
		tries = ACM_MAX_TRIES;
		printk("ACM:%s %s %s\n", __func__, uevent_pkgname, uevent_pic_path);
		while ( tries-- ) {
...
void remove_user_dir(acm_list_node_t *node)
{
	...
	len = VALID_STR_SIZE(strlen(path), ACM_DMD_PIC_PATH - 1) - 1;
