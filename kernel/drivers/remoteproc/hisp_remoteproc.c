...
#define ISP_2_PIPE_FLAG (1 << 20)
    ...
    switch(isplogic_type){
        ...
        case ISP_FPGA:
            ...
            pctrl_stat10 = readl((volatile void __iomem*)(dev->reg[PCTRL] + PCTRL_PERI_STAT10_ADDR));/*lint !e732 */
...
static int hisp_regaddr_init(struct platform_device *pdev)
{
    ...
    for (i = 0; i < HISP_MIN(HISP_MAX_REGTYPE, rproc_dev->reg_num); i ++) {/*lint !e574 */
        ...
        pr_info("[%s] Hisp SOC %d.Resource.%pK.(n.%s, s.0x%llx, e.0x%llx, f.0x%lx) > %pK\n",
            __func__, i, rproc_dev->r[i], rproc_dev->r[i]->name, (unsigned long long)rproc_dev->r[i]->start,
            (unsigned long long)rproc_dev->r[i]->end, rproc_dev->r[i]->flags, rproc_dev->reg[i]);
    }
