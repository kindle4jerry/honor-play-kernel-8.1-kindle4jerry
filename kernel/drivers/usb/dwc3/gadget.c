...
#ifdef CONFIG_USB_DWC3_NOV
		dep->complete_count = 0;
		dep->twice_flag = 0;
#endif
...
#ifdef CONFIG_USB_DWC3_NOV
		if ((dep->twice_flag) && (usb_endpoint_xfer_isoc(dep->endpoint.desc))) {
			dep->twice_flag = 0;
			req = next_request(&dep->req_queued);
			list_del(&req->list);
			dwc3_prepare_trbs(dep, start_new);
...
static int __dwc3_gadget_ep_queue(struct dwc3_ep *dep, struct dwc3_request *req)
{
	...
	if (usb_endpoint_xfer_isoc(dep->endpoint.desc) &&
		...
		ret = __dwc3_gadget_kick_transfer(dep, dep->resource_index, false);
		goto out;
	}
...
#ifdef CONFIG_USB_DWC3_NOV
	if (usb_endpoint_xfer_isoc(dep->endpoint.desc) && (dep->complete_count >= 2)) {
		dep->twice_flag = 1;
		dep->flags |= DWC3_EP_PENDING_REQUEST;
		return 0;
	}
