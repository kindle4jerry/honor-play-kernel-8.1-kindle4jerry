...
static void combophy_firmware_write(void)
{
	...
	*/
	...
	if (hisi_dwc3_is_es()) {
		printk(KERN_ERR "[USB.DBG] in es platform!\n");
...
#ifdef COMBOPHY_FW_UPDATE
	if (tca_dev.update_combophy_firmware != 0)
...
void pd_event_hander(pd_event_t *event)
{
	...
	printk(HISI_TCA_DEBUG"[%s]IRQ[%d]MODEcur[%d]new[%d]DEV[%d]ORIEN[%d]hifi_first_flag[%d]\n",
	...
	if(TCPC_NC == event->mode_type) {
		...
		if (hifi_first_flag) {
			...
			ret = usbctrl_status_update(event->mode_type, event->dev_type, event->typec_orien);
			if (ret) {
				printk(HISI_TCA_DEBUG"[%s]usbctrl_status_update failed[%d]n", __func__, ret);
			}
		} else {
