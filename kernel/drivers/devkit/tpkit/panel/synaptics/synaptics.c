			       ...
			       struct ts_kit_device_data *chip_data)
{
	...
	if (retval) {
		...
		TS_LOG_ERR("Not define device synaptics3718_Tp_Pressure_flag in Dts, use fault value \n");
	}else{
					  ...
					  struct ts_fingers *info)
{
	...
	if(rmi4_data->synaptics_chip_data->synaptics3718_Tp_Pressure_flag == 1){
		...
		if (retval < 0) {
			TS_LOG_ERR("Failed to read data29: %d\n", retval);
			z = 1;
		}
		else {
			z = (int) extra_data->data29_data[0] << 0 | (int) extra_data->data29_data[1] << 8;
			z = z / 97;
		}
		z = z > 0 ? z : 1;
		if ( z > SYNAPTICS_FORCE_LEVEL_MAX)
			z = SYNAPTICS_FORCE_LEVEL_MAX;
	}
	else{
