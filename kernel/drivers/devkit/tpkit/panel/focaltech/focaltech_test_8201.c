...
static int focal_8201_get_open_test_data(struct focal_test_8201 *ft8201, int *data, size_t size, unsigned int chl_x, unsigned int chl_y);
...
static unsigned char  get_current_addr(struct focal_test_8201 *ft8201)
{
	if (!ft8201) {
		TS_LOG_ERR("%s: ft8201 is Null\n", __func__);
		return -ENODEV;
...
static u8 get_master_addr(struct cs_info_packet *cspt)
{
	if (!cspt) {
		TS_LOG_ERR("%s: cspt is Null\n", __func__);
		return -EINVAL;
...
static u8 get_slave_addr(struct cs_info_packet *cspt)
{
	if (!cspt) {
		TS_LOG_ERR("%s: cspt is Null\n", __func__);
		return -EINVAL;
...
static u8 get_master_tx(struct cs_info_packet *cspt)
{
	if (!cspt) {
		TS_LOG_ERR("%s: cspt is Null\n", __func__);
		return -EINVAL;
...
static u8 get_master_rx(struct cs_info_packet *cspt)
{
	if (!cspt) {
		TS_LOG_ERR("%s: cspt is Null\n", __func__);
		return -EINVAL;
...
static u8 get_slave_tx(struct cs_info_packet *cspt)
{
	if (!cspt) {
		TS_LOG_ERR("%s: cspt is Null\n", __func__);
		return -EINVAL;
...
static u8 get_slave_rx(struct cs_info_packet *cspt)
{
	if (!cspt) {
		TS_LOG_ERR("%s: cspt is Null\n", __func__);
		return -EINVAL;
	...
	if (CS_LEFT_RIGHT == get_cs_direction(ft8201->cs_info)) {
		...
		for ( row = 0; row < get_master_tx(ft8201->cs_info) + get_slave_tx(ft8201->cs_info); ++row) {
			for ( col = 0; col < get_master_rx(ft8201->cs_info); ++col) {
				...
				if (row >= split_tx) {
					//down data
					buffer_cated[row * get_master_rx(ft8201->cs_info) + col] = buffer_down[relative_tx * get_master_rx(ft8201->cs_info) + col];
				} else {
					//up data
					buffer_cated[row * get_master_rx(ft8201->cs_info) + col] = buffer_up[row * get_master_rx(ft8201->cs_info) + col];
				}
			}
		}
...
static int focal_8201_get_cb_data_format(struct focal_test_8201 *ft8201, int *data, size_t size, unsigned int chl_x, unsigned int chl_y)
{
	...
	/*judge ic type is cascade connection*/
	if (chip_has_two_heart(ft8201)) {
		buffer_master = kzalloc((master_tx + 1) * master_rx * sizeof(int), GFP_KERNEL);
		if (!buffer_master) {
			TS_LOG_ERR("%s:alloc mem fail\n", __func__);
			return -ENOMEM;
		}
		buffer_slave = kzalloc((slave_tx + 1) * slave_rx * sizeof(int), GFP_KERNEL);
		if (!buffer_slave) {
			TS_LOG_ERR("%s:alloc mem fail\n", __func__);
			goto release_master;
		}
		...
		/*move to master addr,work on master ic*/
		work_as_master(&mgr);
		ret = focal_get_cb_data_format(buffer_master, master_tx * master_rx, master_tx, master_rx);
		if (ret) {
		...
		/*move to slave addr,work on slave ic*/
		work_as_slave(&mgr);
		ret = focal_get_cb_data_format(buffer_slave, slave_tx * slave_rx, slave_tx, slave_rx);
		if (ret) {
			TS_LOG_ERR("%s:get cb data fail, ret=%d\n", __func__, ret);
			goto release_slave;
		}
	...
	struct focal_test_result *srcresult, struct focal_test_result **result, char test_num)
{
	...
	for (i = 0; i < chl_x * chl_y ; i++) {
		if (i < chl_x) {
			test_result->values[i + cb_data_size] = 0;
		} else {
			deviation = focal_abs(srcresult->values[i] - srcresult->values[i - chl_x]);
			max = focal_get_max(srcresult->values[i], srcresult->values[i - chl_x]);
			if (0 == max) {
				TS_LOG_ERR("%s:CB y value max is zero,not normal value return fail\n",__func__);
				return -EINVAL;
			}
			test_result->values[i + cb_data_size] = 100 * deviation / max;
		}
	}
	...
	for (i = 0; i < cb_data_size; i++) {
		if (test_result->values[i + cb_data_size] > cb_uniformity_y) {
			test_result->result = false;
			tp_cap_test_status = PANEL_REASON;
			strncpy(tp_test_failed_reason, "-panel_reason", TP_TEST_FAILED_REASON_LEN);
...
static int focal_8201_get_open_test_data(struct focal_test_8201 *ft8201, int *data, size_t size, unsigned int chl_x, unsigned int chl_y)
{
	...
	read_size = size;
	open_data = kzalloc(read_size * sizeof(int), GFP_KERNEL);
	if (!open_data) {
		TS_LOG_ERR("%s:alloc mem fail\n", __func__);
		return -ENOMEM;
	}
	/*cb main function*/
	ret = focal_8201_get_cb_data_format(ft8201, open_data, read_size, chl_x, chl_y);
	if (ret) {
		TS_LOG_ERR("%s:get cb data fail, ret=%d\n", __func__, ret);
		goto free_open_data;
	}
	...
	struct focal_test_params *params)
{
	...
	ret = ts_kit_parse_csvfile(file_path, FTS_OPEN_TEST_CSV, data_buf, 1, 1);
	if (ret) {
	...
	ret = ts_kit_parse_csvfile(file_path, FTS_RAWDATA_TEST_CSV, data_buf, 1, 2);
	if (ret) {
		TS_LOG_ERR("%s: Failed get %s \n", __func__, DTS_OPEN_TEST_CB_MIN);
		return;
	}
	...
	ret = ts_kit_parse_csvfile(file_path, FTS_CB_UNIFORMITY_TEST_CSV, data_buf, 1, 2);
	if (ret) {
		TS_LOG_ERR("%s: Failed get %s \n", __func__, DTS_OPEN_TEST_CB_MIN);
		return;
	}
	...
	ret = ts_kit_parse_csvfile(file_path, FTS_CB_INCREASE_CSV, data_buf, 1, 2);
	if (ret) {
		TS_LOG_ERR("%s: Failed get %s \n", __func__, DTS_OPEN_TEST_CB_MIN);
		return;
	}
