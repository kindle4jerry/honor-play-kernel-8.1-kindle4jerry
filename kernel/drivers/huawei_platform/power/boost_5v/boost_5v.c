...
static int boost_5v_output(int value)
{
	...
	if (boost_5v_use_common_pmic) {
		...
		if (ret) {
			hwlog_info("%s pmic enable boost fail(ret = %d)\n", __func__, ret);
			return -1;
		}
	} else {
		gpio_set_value(gpio_5v_boost, value);
		hwlog_info("%s: set gpio_5v_boost(%d)\n", __func__, value);
	}
	hwlog_info("%s boost 5v output success!\n", __func__);
	return 0;
}
...
{
	hwlog_info("%s: The module %s set 5V boost to(%d)\n", __func__, module_name_strings(type), enable);
...
static int boost_5v_gpio_init(struct device_node *np)
{
	...
	hwlog_info("%s: gpio_5v_boost = %d\n", __func__, gpio_5v_boost);
	if (!gpio_is_valid(gpio_5v_boost)) {
		hwlog_err("gpio_5v_boost is not valid\n");
		ret = -EINVAL;
		return ret;
	}
	ret = gpio_request(gpio_5v_boost, "gpio_5v_boost");
	if (ret) {
		hwlog_err("could not request gpio_5v_boost\n");
		ret = -ENOMEM;
		return ret;
	}
				  ...
				  const char *buf, size_t count)
{
	...
	if (strstr(saved_command_line, "androidboot.swtype=factory")) {
		...
		hwlog_info("%s set 5V boost success by sys class\n", __func__);
	} else
		hwlog_info("%s only factory version can use sys class to ctrl 5V boost\n", __func__);
...
static int boost_5v_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(np, "boost_5v_use_common_pmic", &boost_5v_use_common_pmic)) {
		hwlog_info("%s use gpio ctrl 5v boost(default)\n", __func__);
	...
	if(!boost_5v_use_common_pmic) {
		...
		if (ret) {
			hwlog_info("%s boost 5v gpio init fail\n", __func__);
			return -1;
		}
	}
	...
	if (ret) {
		hwlog_err("can't create boost_5v sysfs entries\n");
		return -1;
	}

	hwlog_info("boost 5v probe ok.\n");
	return 0;
}
