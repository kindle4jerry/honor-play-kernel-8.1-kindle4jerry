...
static void direct_charge_double_56k_cable_detect(struct direct_charge_device *di)
{
	int ret;
...
static void scp_adaptor_and_cable_detect(struct direct_charge_device *di)
{
	...
	direct_charge_double_56k_cable_detect(di);
	if (SUCC == ret)
	{
...
static void direct_charge_fault_work(struct work_struct *work)
{
	...
	case DIRECT_CHARGE_FAULT_VDROP_OVP:
		...
		break;
	case DIRECT_CHARGE_FAULT_AC_OVP:
		hwlog_err("AC ovp happened!\n");
		snprintf(buf, sizeof(buf), "ac ovp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_AC_OVP, buf);
		break;
	case DIRECT_CHARGE_FAULT_VBAT_OVP:
		hwlog_err("vbat ovp happened!\n");
		snprintf(buf, sizeof(buf), "vbat ovp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_VBAT_OVP, buf);
		break;
	case DIRECT_CHARGE_FAULT_IBAT_OCP:
		hwlog_err("ibat ocp happened!\n");
		snprintf(buf, sizeof(buf), "ibat ocp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_IBAT_OCP, buf);
		break;
	case DIRECT_CHARGE_FAULT_IBUS_OCP:
		hwlog_err("ibus ocp happened!\n");
		snprintf(buf, sizeof(buf), "ibus ocp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_IBUS_OCP, buf);
		break;
	default:
