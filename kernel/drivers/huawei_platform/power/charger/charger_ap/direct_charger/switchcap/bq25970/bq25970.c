...
static int bq25970_charge_init(void)
{
	...
	if(ret)
	{
		hwlog_err("%s: set init parameters error \n", __func__);
		return -1;
	}
...
static int bq25970_charge_exit(void)
{
	...
	ret = bq25970_write_mask(BQ2597X_CONTROL_REG, BQ2597X_REG_RST_MASK, BQ2597X_REG_RST_SHIFT, BQ2597X_REG_RESET);
...
static int bq25970_batinfo_init(void)
{
	int ret = 0;
	ret = bq25970_chip_init();
	ret |= bq25970_adc_enable(BQ2597X_ADC_ENABLE);
	if(ret)
	{
...
#if 0
	ret = loadswitch_ops_register(&bq25970_sysinfo_ops);
	if (ret)
	{
		hwlog_err("register switchcap ops failed!\n");
		goto bq25970_fail_2;
	}
	ret = batinfo_ops_register(&bq25970_batinfo_ops);
	if (ret)
	{
