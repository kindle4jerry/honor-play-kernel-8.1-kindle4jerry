...
static int wireless_charge_boost_control(struct wireless_charge_device_info *di)
{
	...
	if (di->tx_vout_max != di->rx_cap->fast_vout_max) {
		...
		blocking_notifier_call_chain(&di->wireless_charge_evt_nh,
				CHARGER_TYPE_WIRELESS, &di->tx_vout_max);
	}
FuncEnd:
...
static void wireless_charge_iout_control(struct wireless_charge_device_info *di)
{
	...
	if (di->tx_vout_max - vrect > RX_VRECT_LOW_TH) {
		hwlog_err("%s: tx_vout_max(%dmV)-vrect(%dmV)> %dmV, decrease rx_iout %dmA \n",
			__func__, di->tx_vout_max, vrect, RX_VRECT_LOW_TH, di->rx_iout_step);
...
static int wireless_charge_check_tx_disconnect(struct wireless_charge_device_info *di)
{
	...
	if (!wireless_charge_check_tx_exist()){
		if (!di->standard_tx && di->iout_avg <= RX_IOUT_MIN) {
			hwlog_err("%s: delay %dms to report disconnect event\n", __func__, DISCONN_WORK_DELAY_MS);
			...
			(struct device_node* np, struct wireless_charge_device_info *di)
{
	...
	if ((array_len <= 0) || (array_len % WIRELESS_ICTRL_TOTAL != 0)) {
		...
		if (!di->iout_ctrl_data.ictrl_para) {
			di->iout_ctrl_data.ictrl_para_level = 0;
			hwlog_err("%s: alloc tx_cap failed\n", __func__);
			return;
		}
		...
		if (ret) {
			...
			for (i = 0; i < di->iout_ctrl_data.ictrl_para_level; i++) {
				di->iout_ctrl_data.ictrl_para[i].iout_min = temp_para[(int)(WIRELESS_ICTRL_IOUT_MIN + WIRELESS_ICTRL_TOTAL * i)];
				di->iout_ctrl_data.ictrl_para[i].iout_max = temp_para[(int)(WIRELESS_ICTRL_IOUT_MAX + WIRELESS_ICTRL_TOTAL * i)];
				di->iout_ctrl_data.ictrl_para[i].iout_set = temp_para[(int)(WIRELESS_ICTRL_IOUT_SET + WIRELESS_ICTRL_TOTAL * i)];
				hwlog_info("wireless_iout_ctrl_para[%d], iout_min: %d iout_max: %d iout_set: %d\n",
