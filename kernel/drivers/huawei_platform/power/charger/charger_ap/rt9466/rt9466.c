...
#define RT9466_DRV_VERSION	"1.0.1_HUAWEI"
...
static int __rt9466_set_ichg(struct rt9466_info *info, u32 ichg)
{
	...
	if (ichg < 900 && !info->ieoc_wkard) { /* 900mA */
		ret = __rt9466_set_ieoc(info, info->ieoc + 100);
...
static int rt9466_enable_otg(int en)
{
	...
	/* Set OTG_OC to 500mA */
	ret = rt9466_set_boost_current_limit(500);
	if (ret < 0) {
...
static int rt9466_run_aicl(struct ico_input *input, struct ico_output *output)
{
	int ret = 0;
	struct rt9466_info *info = i2c_get_clientdata(g_rt9466_i2c);
	...
	if (input->charger_type != CHARGER_TYPE_STANDARD)
		return -1;

	ret = __rt9466_run_aicl(info);
	if (ret < 0)
		return ret;
	output->input_current = info->aicr_limit;
	return 0;
}
...
static struct charge_device_ops rt9466_ops = {
	.set_charger_hiz = rt9466_enable_hz,
