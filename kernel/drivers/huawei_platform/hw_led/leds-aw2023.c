...
#define MAX_RISE_TIME_MS				15U
#define MAX_HOLD_TIME_MS				15U
#define MAX_FALL_TIME_MS				15U
#define MAX_OFF_TIME_MS					15U
...
static int aw2023_write(struct aw2023_led *led, u8 reg, u8 val)
{
	int ret = -EINVAL, retry_times = 0;
        if (NULL == led) {
            hwlog_err("%s-%d null pointer error!\n", __func__, __LINE__);
            return ret;
        }
...
static void aw2023_brightness_work(struct work_struct *work)
{
	...
	if (!(val&AW2023_CHIP_ENABLE_MASK)) {
        ...
        hwlog_info("aw2023 brightness set! brightness=%d,led_id=%d\n",led->cdev.brightness,led->id);
...
static void aw2023_led_blink_set(struct aw2023_led *led, unsigned long blinking)
{
	...
	led->cdev.brightness = blinking ? led->cdev.max_brightness : 0;
        hwlog_info("aw2023 blink set! blinking=%d,brightness=%d\n",blinking,led->cdev.brightness);
	if (blinking > 0) {
		aw2023_write(led, AW2023_REG_GCR2, led->pdata->imax);
		aw2023_write(led, AW2023_REG_LCFG0 + led->id,
			     ...
			     const char *buf, size_t len)
{
	...
	rc = sscanf(buf, "%d %d %d %d",
        ...
        hwlog_info("aw2023_led_time,rise=%d,hold=%d,fall=%d,off=%d\n",rise_time_ms,hold_time_ms,fall_time_ms,off_time_ms);
	mutex_lock(&led->pdata->led->lock);
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (ret) {
        ...
        hwlog_info("aw2023_led probe success!\n");
	return 0;
...
{
	struct aw2023_led *led_array = NULL;
        int i, parsed_leds =0;
        if (NULL == client) {
            hwlog_err("%s null pointer error!\n", __func__);
            return -EINVAL;
	...
	led_array = i2c_get_clientdata(client);
	parsed_leds = led_array->num_leds;
