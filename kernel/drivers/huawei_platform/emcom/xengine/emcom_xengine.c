...
int Emcom_Xengine_StopAccUid(uint8_t *pdata, uint16_t len)
{
	...
	if(len != sizeof(uid_t))
	{
		EMCOM_LOGI("Emcom_Xengine_StopAccUid: len£º%d is illegal", len);
		return -EINVAL;
	}
...
int Emcom_Send_KeyPsInfo(uint8_t *pdata, uint16_t len)
{
	...
	{
		EMCOM_LOGE("Emcom_Send_KeyPsInfo: len£º%d is illegal", len);
		return -EINVAL;
	}
...
int Emcom_Xengine_StartUdpReTran(uint8_t *pdata, uint16_t len)
{
	...
	if(len != sizeof(uid_t))
	{
		EMCOM_LOGI("Emcom_Xengine_StartUdpReTran: len£º%d is illegal", len);
		return -EINVAL;
	}
	...
	EMCOM_LOGI("Emcom_Xengine_StartUdpReTran: uid£º%d ", uid);
	g_UdpRetranUid = uid;
	return 0;
}
...
int Emcom_Xengine_StartFastSyn(uint8_t *pdata, uint16_t len)
{
	...
	if(len != sizeof(uid_t))
	{
		EMCOM_LOGI(" Emcom_Xengine_StartFastSyn: len£º%d is illegal", len);
		return -EINVAL;
	}
	...
	EMCOM_LOGI(" Emcom_Xengine_StartFastSyn: uid£º%d ", uid);
	g_FastSynUid = uid;
	return 0;
}
