			      ...
			      const char *buf, size_t count)
{
	...
	if (value > 0) {
		...
		pr_err("%s: static_leakage = %lu\n", __func__, dsu->static_leakage);
		...
		dsu->downsize_threshold *= dsu->static_leakage;
		dsu->downsize_threshold /= dsu->dram_energy_per_mb;
		pr_err("%s: downsize_threshold = %lu\n", __func__, dsu->downsize_threshold);
			      ...
			      const char *buf, size_t count)
{
	...
	if (dsu->up_polling_ms && value > 0 && value <= 10) {
		...
		dsu->down_polling_ms = dsu->down_interval * devfreq->profile->polling_ms;  //dsu->devfreq_profile->polling_ms
		ret = count;
	}else{
		ret = -EINVAL;
	}
					 ...
					 unsigned long *portions)
{
	...
	//pr_err("%s: portion_active = %lu  *portions = %lu\n", __func__, portion_active, *portions);
		...
		unsigned long mode, void *_unused)
{
	...
	switch (mode) {
		case L3C_ACP_ENABLE:
			//pr_err("%s: acp stop dsu pctrl\n", __func__);
			stop_dsu_pctrl(dsu);
			break;
		case L3C_ACP_DISABLE:
			//pr_err("%s: acp start dsu pctrl\n", __func__);
			start_dsu_pctrl(dsu);
			break;
		default:
			break;
...
static int dsu_pctrl_devfreq_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "DSU R%d portion control device registered.\n",
