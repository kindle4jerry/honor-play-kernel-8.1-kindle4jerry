...
#define HISI_PMIC_VIBRATOR_SPEL_TIME_VALUE4     0x30
#define HISI_PMIC_VIBRATOR_SPEL_TIME_REG5       0xA055
#define HISI_PMIC_VIBRATOR_SPEL_TIME_VALUE5     0x23
...
#define GATE_VOLTAGE1_A         113965
#define GATE_VOLTAGE1_B         130000

#define GATE_VOLTAGE2_A         74064
#define GATE_VOLTAGE2_B         6024000

#define GATE_VOLTAGE3_A         23741
#define GATE_VOLTAGE3_B         1298000
...
struct hisi_pmic_vibrator_dev {
	...
	u32 pmic_vibrator_gpio_en;
	u32 pmic_vibrator_power_flage;
...
static int hisi_pmic_vibrator_power_on_off(int pmic_vibrator_power_onoff)
{
	...
	if(g_vdev->pmic_vibrator_power_flage) {
#ifdef CONFIG_BOOST_5V
...
static s32 hisi_pmic_vibrator_set_strength(void)
{
	...
	//gate1
	val = (g_vdev->pmic_vibrator_strength * GATE_VOLTAGE1_A + GATE_VOLTAGE1_B) / 1000000; /*lint !e647 */
	gate1_l = val & 0xff;
	gate1_h = val >> 8;
	hisi_pmic_vibrator_write_u8(gate1_l, HISI_PMIC_VIBRATOR_GATE_VOLTAGE1_L);
	hisi_pmic_vibrator_write_u8(gate1_h, HISI_PMIC_VIBRATOR_GATE_VOLTAGE1_H);
	//gate2
	val = (g_vdev->pmic_vibrator_strength * GATE_VOLTAGE2_A - GATE_VOLTAGE2_B) / 1000000; /*lint !e647 */
	gate2_l = val & 0xff;
	gate2_h = val >> 8;
	hisi_pmic_vibrator_write_u8(gate2_l, HISI_PMIC_VIBRATOR_GATE_VOLTAGE2_L);
	hisi_pmic_vibrator_write_u8(gate2_h, HISI_PMIC_VIBRATOR_GATE_VOLTAGE2_H);
	//gate3
	val = (g_vdev->pmic_vibrator_strength * GATE_VOLTAGE3_A - GATE_VOLTAGE3_B) / 1000000; /*lint !e647 */
	gate3_l = val & 0xff;
	gate3_h = val >> 8;
	hisi_pmic_vibrator_write_u8(gate3_l, HISI_PMIC_VIBRATOR_GATE_VOLTAGE3_L);
	hisi_pmic_vibrator_write_u8(gate3_h, HISI_PMIC_VIBRATOR_GATE_VOLTAGE3_H);
...
static enum hrtimer_restart hisi_pmic_vibrator_timer_func(struct hrtimer *timer)
{
	...
	schedule_work(&vdev->vibrator_off_work);
...
{
	uint64_t val = 0;
	u32 ladd_reg, duty_reg, time_reg0, time_reg1;
	u32 i, idx;
	...
	for(i = 0; i < HISI_PMIC_VIBRATOR_SPEL_TIME_REG_NUM; i++) {
		time_reg0 = HISI_PMIC_VIBRATOR_SPEL_TIME_REG0 + i;
		time_reg1 = HISI_PMIC_VIBRATOR_SPEL_TIME_REG6 + i;

		hisi_pmic_vibrator_write_u8(val, time_reg0);
		hisi_pmic_vibrator_write_u8(val, time_reg1);
	}
	hisi_pmic_vibrator_write_u8(HISI_PMIC_VIBRATOR_SPEL_TIME_VALUE4, HISI_PMIC_VIBRATOR_SPEL_TIME_REG4);
	hisi_pmic_vibrator_write_u8(HISI_PMIC_VIBRATOR_SPEL_TIME_VALUE5, HISI_PMIC_VIBRATOR_SPEL_TIME_REG5);
...
static s32 hisi_pmic_vibrator_parse_dt(struct hisi_pmic_vibrator_dev *vdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "vibrator-gpio-en", &temp);
	if (ret < 0) {
		dev_info(dev, "get gpio_en fail,not uesed gpio_en for 5v!\n");
		vdev->pmic_vibrator_power_flage = POWER_NOTUSED_BOOST_5V;
	}else {
		vdev->pmic_vibrator_gpio_en = temp;
		dev_info(dev, "gpio_en is %d\n",vdev->pmic_vibrator_gpio_en);
		vdev->pmic_vibrator_power_flage = POWER_USED_BOOST_5V;
	}
