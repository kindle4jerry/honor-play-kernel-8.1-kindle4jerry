...
static int get_coul_chip_temp(void)
{
    ...
    if(g_coul_core_ops && g_coul_core_ops->get_chip_temp) {
        ...
        hwlog_info("%s:chip_temp=%d\n",__func__,chip_temp);
        return chip_temp;
    } else {
        hwlog_err("%s:get_coul_chip_temp failed.",__func__);
        return -1;
    }
}
...
static int hw_coul_operate_nv(struct hw_coul_nv_info *info, enum nv_operation_type type)
{
    ...
    if(nve.valid_size > NVE_NV_DATA_SIZE) {
        hwlog_err("[%s]struct info is too big for nve!\n", __func__);
        return ret;
    }
    ...
    if (NV_WRITE_TYPE == type) {
        ...
        if (ret) {
            hwlog_err("[%s]write nv failed, ret = %d\n", __func__, ret);
        }
    } else {
        nve.nv_operation = NV_READ;
        ret = hisi_nve_direct_access(&nve);
        if (ret) {
            hwlog_err("[%s]read nv failed, ret = %d\n", __func__, ret);
        } else {
...
static void hw_coul_get_nv(struct smartstar_coul_device *di)
{
    ...
    if (batt_backup_nv_flag && 0 == batt_backup_nv_read) {
        ...
        if (NV_NOT_DEFINED == ret) {
            hwlog_err("battery backup nv not defined, disable battery backup nv flag\n");
            batt_backup_nv_flag = 0;
        } else if (NV_OPERATE_SUCC == ret) {
            hwlog_info("read battery backup nv info succ\n");
...
static void hw_coul_update_chargecycles(struct smartstar_coul_device *di)
{
    if (batt_backup_nv_flag && 1 == batt_backup_nv_read) {
        ...
        hwlog_info("battery backup chargecycle=%d, added=%d\n", batt_backup_nv_info.charge_cycles, di->batt_soc_real/10 - di->charging_begin_soc/10);
    }
    return;
}
...
static void hw_coul_save_nv(struct smartstar_coul_device *di, struct ss_coul_nv_info *pinfo)
{
    if (batt_backup_nv_flag && 1 == batt_backup_nv_read) {
        ...
        if (NV_OPERATE_SUCC == hw_coul_operate_nv(&batt_backup_nv_info, NV_WRITE_TYPE)) {
            hwlog_info("write battery backup nv info succ\n");
        }
    }
    return;
}
...
static void update_polar_params(struct smartstar_coul_device *di)
{
    ...
    sync_sample_info();
    polar_params_calculate(&di->polar, ocv_soc_mv,
                           vol_now/UVOLT_PER_MVOLT, -curr_now/UA_PER_MA);
#ifdef POLAR_INFO_DEBUG
    hwlog_info("vol:%d,curr:%d,p_vol:%ld,curr_5s:%d,curr_peak:%d,p_ocv:%d\n",
            vol_now/UVOLT_PER_MVOLT, -curr_now/UA_PER_MA,
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    case COUL_SYSFS_GAUGELOG_HEAD:
        return snprintf(buf, PAGE_SIZE, "ss_VOL  ss_CUR  ss_ufSOC  ss_SOC  SOC  ss_RM  ss_FCC  ss_UUC  ss_CC  ss_dRC  pdFlag Temp  ss_OCV   rbatt  fcc    ocv_level  fcc_flag   ocv_old    p_vol   p_cur    err_a    polar_vol   avg_cur    ");
    case COUL_SYSFS_GAUGELOG:
        ...
        snprintf(buf, PAGE_SIZE, "%-6d  %-6d  %-8d  %-6d  %-3d  %-5d  %-6d  %-6d  %-5d  %-6d  %-5d  %-4d  %-7d  %-5d  %-5d  %-9d   %-8d   %-7d    %-5d    %-5d    %-5d    %-9ld    %-7d    ",
                    ...
                    di->polar.ocv_old, di->polar.ori_vol, di->polar.ori_cur, di->polar.err_a, di->polar.vol, di->polar.curr_5s);
...
static void coul_core_get_dts(struct smartstar_coul_device *di)
{
    ...
    if (of_property_read_u32(np, "batt_backup_nv_flag", &batt_backup_nv_flag)) {
        hwlog_err("dts:get batt_backup_nv_flag fail, use default value!\n");
    }
    hwlog_info("batt_backup_nv_flag = %d\n", batt_backup_nv_flag);
...
static int hisi_coul_resume(struct platform_device *pdev)
{
	...
	/*get sleep current*/
    /* ma = ua/1000 = uas/s/1000 = uah*3600/s/1000 = uah*18/(s*5) */
    get_resume_polar_info(sleep_cc, sr_sleep_time);
    start_polar_sample();
#endif
