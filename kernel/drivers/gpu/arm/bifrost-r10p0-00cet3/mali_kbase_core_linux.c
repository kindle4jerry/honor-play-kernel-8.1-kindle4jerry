...
#if 0
static int kbase_api_map_iova(struct kbase_context *kctx,
		union kbase_ioctl_mem_iova *iova)
{
	return -EINVAL;
}

static int kbase_api_umap_iova(struct kbase_context *kctx,
		union kbase_ioctl_mem_iova *iova)
{
	return -EINVAL;
}
...
#if 0
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_MAP_IOVA,
				kbase_api_map_iova,
				union kbase_ioctl_mem_iova);
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_UMAP_IOVA,
				kbase_api_umap_iova,
				union kbase_ioctl_mem_iova);
#endif
	...
	do {
		while (kbase_event_dequeue(kctx, &uevent)) {
			...
			if (wait_event_interruptible(kctx->event_queue,
					kbase_event_pending(kctx)) != 0)//lint !e666
				return -ERESTARTSYS;
		}
...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	if (!kbdev->crgreg) {
		dev_err(kbdev->dev, "Can't remap sys crg register window on platform hi3650\n");
		err = -EINVAL;
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	if (np) {
		...
		{
			dev_err(kbdev->dev,
			"failed to get gpu_outstanding resource.\n");
		}
	}
	else {
		dev_err(kbdev->dev,
		"NOT FOUND device node arm,mali-midgard!\n");
	}
	...
	kbdev->ipa_ctx = kbase_dynipa_init(kbdev);
	if (!kbdev->ipa_ctx) {
			dev_err(kbdev->dev,
			"GPU HW bound detection sub sys initialization failed\n");
	}
	INIT_WORK(&kbdev->bound_detect_work, mali_kbase_devfreq_detect_bound_worker);
#endif
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0006)) {
		...
		//read G3DHPMBYPASS and set it's [3:0] and [19:16]bit to 0,enable G3D HPM hardware status contrl
		value = readl(kbdev->pmctrlreg + G3DHPMBYPASS) & MASK_G3DHPMBYPASS;
		writel(value, kbdev->pmctrlreg + G3DHPMBYPASS);

		//read G3DAUTOCLKDIVBYPASS and set it's [0]bit to 0,enable G3D auto clkdiv
		...
		//read VS_CTRL_2 and set it's [26]bit to 1,enable L2 reduce freq when it IDLE
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0012)) {
		/*for sigurd,set PCTRL93 bit16/17/18 to 1,enable mem auto shutdown*/
		unsigned int value = 0;
