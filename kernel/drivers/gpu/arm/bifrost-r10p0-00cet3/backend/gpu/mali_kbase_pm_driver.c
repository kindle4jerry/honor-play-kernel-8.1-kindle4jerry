...
{
	struct kbase_context *kctx_i, *n;

	dev_warn(kbdev->dev,"kbase_jd_atoms_show start\n");

	spin_lock(&kbdev->ctx_list_head_lock);
	list_for_each_entry_safe(kctx_i, n, &kbdev->ctx_list_head, ctx_list_entry) {
		struct kbase_jd_atom *atoms;
		int i;

		atoms = kctx_i->jctx.atoms;
	...
	dev_warn(kbdev->dev,"kbase_jd_atoms_show end\n");
}
...
//	timeout = jiffies + PM_TIMEOUT;
	...
	/* Wait for cores */
	ret = wait_event_interruptible_timeout(kbdev->pm.backend.gpu_in_desired_state_wait,
			kbdev->pm.backend.gpu_in_desired_state, PM_TIMEOUT);
	...
	if (ret <= 0) {
		...
		dev_err(kbdev->dev, "\tTiler =%016llx\n",
				kbdev->pm.backend.desired_tiler_state);
		dev_err(kbdev->dev, "\tshader_inuse_bitmap  =%016llx, shader_needed_bitmap=%016llx\n",
				kbdev->shader_inuse_bitmap, kbdev->shader_needed_bitmap);
		dev_err(kbdev->dev, "\tactive_count=%d, shader_available_bitmap=%016llx\n",
				kbdev->pm.active_count,kbdev->shader_available_bitmap);
		dev_err(kbdev->dev, "\tshader_ready_bitmap=%016llx, shader_transitioning_bitmap=%016llx\n",
                                kbdev->shader_ready_bitmap,kbdev->shader_transitioning_bitmap);
		dev_err(kbdev->dev,"\tfaults_pending=%d, protected_mode=%d\n",
				atomic_read(&kbdev->faults_pending),kbdev->protected_mode);
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0002)) {
		...
		gpu_dsm_report(kbdev, 922002004, "Failed to soft-reset GPU (timed out after %d ms), now attempting a hard reset\n",
											RESET_TIMEOUT);
#endif
		kbdev->error_num.hard_reset++;
		kbdev->error_num.ts = hisi_getcurtime();
...
#ifdef CONFIG_HUAWEI_DSM
		gpu_dsm_report(kbdev, 922002005, "Failed to hard-reset the GPU (timed out after %d ms) !!!!", RESET_TIMEOUT);
#endif
		kbdev->error_num.hard_reset++;
		kbdev->error_num.ts = hisi_getcurtime();
