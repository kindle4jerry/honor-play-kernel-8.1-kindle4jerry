...
void page_fault_worker(struct work_struct *data)
{
	...
	kbdev = container_of(faulting_as, struct kbase_device, as[as_no]);/* [false alarm]: no problem - fortify check */
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			for (i = 0; i < count; i++) {
				...
				if (kbase_is_entry_decompressed(phys[i])) {
					...
					kctx->kbdev->mmu_mode->entry_invalidate(target);
			}
#else
				kctx->kbdev->mmu_mode->entry_set_ate(target,
						phys[i], flags, cur_level);
...
int kbase_mmu_insert_pages_with_scramble_bit_no_flush(struct kbase_context *kctx,
				    const u64 start_vpfn,
				    struct tagged_addr *phys, size_t nr,
	...
	while (remain) {
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			for (i = 0; i < count; i++) {
				...
				{
					kctx->kbdev->mmu_mode->entry_set_ate_scramble_bit(target,
							phys[i], flags, cur_level);
				}
			}
		}
		...
		u64 vpfn, size_t nr, bool sync)
{
	...
	if (err) {
		...
		gpu_dsm_report(kbdev, 922002004, "Flush for GPU page table update did not complete. Issuing GPU soft-reset to recover!!!!");
#endif
...
#ifdef CONFIG_HUAWEI_DSM
				gpu_dsm_report(kbdev, 922002004, "Flush for GPU page table update did not complete. Issueing GPU soft-reset to recover!!!!");
#endif
				kbdev->error_num.soft_reset++;
				kbdev->error_num.ts = hisi_getcurtime();
#ifdef CONFIG_HISI_ENABLE_HPM_DATA_COLLECT
...
#ifdef CONFIG_HUAWEI_DSM
		gpu_dsm_report(kbdev, 922002004, "GPU bus error occurred. For this GPU version we now soft-reset as part of bus error recovery!!!!");
#endif
		kbdev->error_num.soft_reset++;
		kbdev->error_num.ts = hisi_getcurtime();
#ifdef CONFIG_HISI_ENABLE_HPM_DATA_COLLECT
...
#ifdef CONFIG_HUAWEI_DSM
	gpu_dsm_report(kbdev, 922002001, "Unhandled Page fault!!!!");
#endif
...
#ifdef CONFIG_HUAWEI_DSM
		gpu_dsm_report(kbdev, 922002000, "Unhandled page fault. For this GPU version we now soft-reset the GPU as part of page fault recovery.");
#endif
		kbdev->error_num.page_fault++;
		kbdev->error_num.ts = hisi_getcurtime();
#ifdef CONFIG_HISI_ENABLE_HPM_DATA_COLLECT
...
void kbasep_as_do_poke(struct work_struct *work)
{
	...
	kbdev = container_of(as, struct kbase_device, as[as->number]);/* [false alarm]: no problem - fortify check */
