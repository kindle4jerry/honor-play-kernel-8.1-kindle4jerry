...
static const struct snd_pcm_hardware hi6210_hardware_lowlatency_playback =
{
	.info             = SNDRV_PCM_INFO_INTERLEAVED
		| SNDRV_PCM_INFO_NONINTERLEAVED
		| SNDRV_PCM_INFO_MMAP
...
static const struct snd_pcm_hardware hi6210_hardware_lowlatency_capture =
{
	.info             = SNDRV_PCM_INFO_INTERLEAVED,
	.formats          = SNDRV_PCM_FMTBIT_S16_LE,
	.rates            = SNDRV_PCM_RATE_48000,
...
static irq_rt_t hi6210_mb_isr_handle(unsigned short pcm_mode, struct snd_pcm_substream *substream);
#endif
static irq_rt_t hi6210_isr_handle(struct snd_pcm_substream *substream);
...
{
	struct hi6210_pcm_private_data *priv_data = (struct hi6210_pcm_private_data *)data;
	...
	while (!kthread_should_stop()) {
		...
		if (priv_data->pcm_fast_pb_status_open && STATUS_RUNNING == prtd->status) {
			if (is_one_frame_finished(prtd->substream)) {
				hi6210_isr_handle(prtd->substream);
...
static int pcm_thread_start(void)
{
	...
	if (ret)
		loge("set thread schedule priorty failed\n");

	wake_up_process(pdata.pcm_run_thread);
...
/*****************************************************************************
	function name  : hi6210_mb_isr_handle
	...
	Author  :
 *****************************************************************************/
static irq_rt_t hi6210_mb_isr_handle(unsigned short pcm_mode,
		struct snd_pcm_substream *substream)
{
	...
	case SNDRV_PCM_STREAM_PLAYBACK:
		...
		ret = hi6210_isr_handle(substream);
		spin_unlock_bh(&g_pcm_pb_open_lock);
		break;
...
/*****************************************************************************
	...
	output : no
	return  : STATIC int
	called function : hi6210_isr_handle()
	history    :
	Date  :
	Author  :
...
static int hi6210_pcm_notify_set_buf(struct snd_pcm_substream *substream)
{
	...
	/* mail-box send */
	/* trace_dot(APCM,"4",0); */
	if (substream->pcm->device == PCM_DEVICE_FAST) {
...
static irq_rt_t hi6210_notify_recv_isr(void *usr_para, void *mail_handle, unsigned int mail_len)
{
	...
	switch(mail_buf.msg_type) {
		case HI_CHN_MSG_PCM_PERIOD_ELAPSED:
			...
			ret = hi6210_mb_isr_handle(mail_buf.pcm_mode, substream);
			if (ret == IRQ_NH)
...
static int hi6210_pcm_notify_trigger(int cmd, struct snd_pcm_substream *substream)
{
	...
	if ((msg_body.pcm_device == PCM_DEVICE_FAST) && (cmd == SNDRV_PCM_TRIGGER_START)) {
		ret = pcm_set_share_data(msg_body.pcm_mode, msg_body.data_addr, msg_body.data_len);
		if (ret)
...
static int hi6210_pcm_open(struct snd_pcm_substream *substream)
{
	...
	case PCM_DEVICE_FAST:
		if (SNDRV_PCM_STREAM_PLAYBACK == substream->stream) {
			...
			spin_unlock_bh(&g_pcm_pb_open_lock);
			snd_soc_set_runtime_hwparams(substream, &hi6210_hardware_lowlatency_playback);
			...
			pdata.pcm_cp_status_open = (u32)1;
			spin_unlock_bh(&g_pcm_cp_open_lock);
			snd_soc_set_runtime_hwparams(substream, &hi6210_hardware_lowlatency_capture);
			prtd = &pdata.pcm_rtd_capture;
			prtd->substream = substream;
		}
		pcm_thread_start();
