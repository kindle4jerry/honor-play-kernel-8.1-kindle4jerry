...
void av_log_default_callback_use_ALog(void* ptr, int level, const char* fmt, va_list vl)
{
    if (level > av_log_level)
        return;

    char logBuf[LINE_SZ] = {0};
    vsnprintf(logBuf, LINE_SZ, fmt, vl);

    if (level == AV_LOG_ERROR)
    {
        ALOG(LOG_ERROR, LOG_TAG, "%s", logBuf);
    }
    else if (level == AV_LOG_WARNING)
    {
        ALOG(LOG_WARN, LOG_TAG, "%s", logBuf);
    }
    else if (level == AV_LOG_INFO)
    {
        ALOG(LOG_INFO, LOG_TAG, "%s", logBuf);
    }
    else if (level == AV_LOG_VERBOSE)
    {
        ALOG(LOG_VERBOSE, LOG_TAG, "%s", logBuf);
    }
    else if (level > AV_LOG_VERBOSE)
    {
        ALOG(LOG_DEBUG, LOG_TAG, "%s", logBuf);
    }
    else
    {
        ALOG(LOG_FATAL, LOG_TAG, "%s", logBuf);
    }
}
...
static void (*av_log_callback)(void*, int, const char*, va_list) =
    av_log_default_callback_use_ALog;
