...
bool hw_compute_file_contexts_hash(uint8_t c_digest[])
{
    if (access(seopts_file_split[0].value, R_OK) != -1)
    {
        return compute_file_contexts_hash(c_digest, seopts_file_split, sizeof(seopts_file_split) / sizeof(seopts_file_split[0]));
    }
    else
    {
        return compute_file_contexts_hash(c_digest, seopts_file_rootfs, sizeof(seopts_file_rootfs) / sizeof(seopts_file_rootfs[0]));
    }
}
