...
unsigned char *blkid_probe_get_buffer(struct blkid_probe *pr,
	blkid_loff_t off, size_t len)
{
	return (unsigned char *)get_buffer(pr, off, len);
}
...
int probe_exfat(struct blkid_probe *probe, struct blkid_magic *id,
                       unsigned char *buf)
{
    ...
    blkid_set_tag(probe->dev, "UUID", uuid, sizeof(uuid)-1);
    return 0;
}
...
static uint32_t next_cluster(struct blkid_probe * pr,
    const struct exfat_super_block *sb, uint32_t cluster)
{
    ...
    next = (uint32_t *) get_buffer(pr, fat_offset,
        sizeof(uint32_t));
    if (!next)
        return 0;
...
static struct exfat_entry_label *find_label(struct blkid_probe * pr,
    const struct exfat_super_block *sb)
{
    ...
    for (;;) {
        entry = (uint8_t *) get_buffer(pr, offset,
            EXFAT_ENTRY_SIZE);
        if (!entry)
            return NULL;
        if (entry[0] == EXFAT_ENTRY_EOD)
