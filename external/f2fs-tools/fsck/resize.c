...
static int get_new_sb(struct f2fs_super_block *sb, u_int32_t old_crc)
{
	u_int32_t crc = 0;
				...
				struct f2fs_super_block *new_sb)
{
	...
	unsigned int nid, new_max_nid, nat_blocks;
			...
			struct f2fs_super_block *new_sb)
{
	...
	unsigned int nat_blocks, new_max_nid;
	void *nat_block;
	int nid, ret;
	pgoff_t block_off;
	pgoff_t block_addr;
	...
	for (nid = nm_i->max_nid - 1; nid >= 0; nid -= NAT_ENTRY_PER_BLOCK) {
		...
		block_addr = (pgoff_t)(old_nat_blkaddr +
				(seg_off << sbi->log_blocks_per_seg << 1) +
				(block_off & ((1 << sbi->log_blocks_per_seg) - 1))); /*lint !e647 !e701*/

		if (f2fs_test_bit(block_off, nm_i->nat_bitmap)) /*lint !e712 !e747*/
			block_addr += sbi->blocks_per_seg;
		...
		block_addr = (pgoff_t)(new_nat_blkaddr +
				(seg_off << sbi->log_blocks_per_seg << 1) + /*lint !e504 !e701*/
				(block_off & ((1 << sbi->log_blocks_per_seg) - 1))); /*lint !e647 !e701*/
		...
		struct f2fs_super_block *new_sb, unsigned int offset)
{
	...
	block_t sit_blks = get_newsb(segment_count_sit) <<
						(sbi->log_blocks_per_seg - 1); /*lint !e701*/
	struct seg_entry *se;
	block_t blk_addr = 0;
	int ret;
	...
	for (index = 0; index < sit_blks; index++) {
		ret = dev_write_block(sit_blk, (__u64)get_newsb(sit_blkaddr) + index);
		ASSERT(ret >= 0);
	...
	for (segno = 0; segno < TOTAL_SEGS(sbi); segno++) {
		...
		if (ofs != pre_ofs) {
			...
			ret = dev_write_block(sit_blk, (__u64)blk_addr);
			ASSERT(ret >= 0);
			DBG(1, "Write valid sit: %x\n", blk_addr);

			pre_ofs = ofs;
			memset(sit_blk, 0, BLOCK_SZ); /*lint !e669*/
		}
			...
			struct f2fs_super_block *new_sb, unsigned int offset)
{
	...
	ckpt_flags = le32_to_cpu(new_cp->ckpt_flags); /*lint !e732 !e744*/
	if (ckpt_flags & CP_COMPACT_SUM_FLAG)
		ckpt_flags &= ~CP_COMPACT_SUM_FLAG;
	new_cp->ckpt_flags = cpu_to_le32(ckpt_flags);
...
int f2fs_resize(struct f2fs_sb_info *sbi)
{
	...
	if (get_new_sb(new_sb, get_sb(crc)))
		return -1;
