...
void ssa_dump(struct f2fs_sb_info *sbi, int start_ssa, int end_ssa)
{
	...
	for (segno = start_ssa; segno < end_ssa; segno++) {
		sum_blk = get_sum_block(sbi, segno, &seg_type);
		...
		if (seg_type == SEG_TYPE_NODE || seg_type == SEG_TYPE_DATA ||
					seg_type == SEG_TYPE_MAX)
			free(sum_blk);
	}
	close(fd);
}
...
static void dump_data_offset(u32 blk_addr, int ofs_in_node)
{
	...
	ret = dev_read_block(node_blk, (__u64)blk_addr);
	ASSERT(ret >= 0);
	...
	if (node_ofs > 0 && node_ofs <= 2) {
		...
		unsigned int dec = (node_ofs - 4) / (NIDS_PER_BLOCK + 1);
		bidx = node_ofs - 2 - dec;
	} else {
		unsigned int dec = (node_ofs - indirect_blks - 3) / (NIDS_PER_BLOCK + 1);
		bidx = node_ofs - 5 - dec;
	}
