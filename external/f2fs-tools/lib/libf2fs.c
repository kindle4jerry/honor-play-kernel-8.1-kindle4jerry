...
int f2fs_test_bit(u64 nr, const char *p)
{
	int mask;
	char *addr = (char *)p;
...
int f2fs_set_bit(u64 nr, char *addr)
{
	int mask;
	int ret;
...
int f2fs_clear_bit(u64 nr, char *addr)
{
	int mask;
	int ret;
...
static void str2hashbuf(const unsigned char *msg, unsigned int len,
 ...
 */
f2fs_hash_t f2fs_dentry_hash(const unsigned char *name, unsigned int len)
{
	__u32 hash;
