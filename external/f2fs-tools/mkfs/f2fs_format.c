...
static int f2fs_prepare_super_block(void)
{
	...
	u_int32_t segment_count_extent;
	u_int32_t crc = 0;
	int i;
	...
	if(c.extent_sectors)
		segment_count_extent =(c.extent_sectors * c.sector_size -
				zone_align_start_offset) / segment_size_bytes;
	...
	if(c.extent_sectors)
		blocks_for_sit = ALIGN(segment_count_extent, SIT_ENTRY_PER_BLOCK);
	...
	if(c.extent_sectors)
		total_valid_blks_available = (segment_count_extent -
	...
	/* get kernel version */
	if (c.kd >= 0) {
		if (dev_read_version(c.version, (__u64)0, VERSION_LEN) < 0)
		{
	...
	set_sb(crc,crc);
	MSG(0, "Info: update crc successfully (0x0 --> 0x%x)\n", crc);
...
static int f2fs_write_check_point_pack(void)
{
	...
	cp->checkpoint_ver = rand_r(&seed) | 0x1;
...
static int f2fs_write_super_block(void)
{
	...
	if (zero_buff == NULL) {
		MSG(0, "\tError: Calloc Failed for super block!!!\n");
		return -1;
	}
...
static int discard_obsolete_dnode(struct f2fs_node *raw_node, u_int64_t offset)
{
	...
	do {
		...
		if (dev_write(raw_node, offset * F2FS_BLKSIZE, F2FS_BLKSIZE)) {
